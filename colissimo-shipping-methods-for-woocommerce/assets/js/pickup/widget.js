    ...
    window.lpc_callback = function (point) {
        ...
        if ($affectMethodDiv.length === 0) {
            ...
            $affectMethodDiv.find('.lpc_order_affect_relay_information_displayed')
                            .empty()
                            .append(document.createTextNode(point['nom'] + ' (' + point['identifiant'] + ')'))
                            .append('<br>')
                            .append(document.createTextNode(point['adresse1']))
                            .append('<br>')
                            .append(document.createTextNode(point['codePostal'] + ' ' + point['localite']));
        }
    };

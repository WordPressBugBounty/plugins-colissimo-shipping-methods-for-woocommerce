<?php
// phpcs:disable WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- intentional re-use of documented WooCommerce/WordPress core hooks.
...
class InvoiceGenerateAction {
    ...
    public function printOrderTitle($order) {
        ...
        $output = '<h3>' . sprintf(__('[Order #%s]', 'colissimo-shipping-methods-for-woocommerce'),
                                   $order->get_id()) . ' (' . wc_format_datetime($order->get_date_created()) . ')</h3>';
    ...
    public function printProductsInHTML($order) {
        ...
        $output .= '<th class="td" scope="col" style="text-align:left;">' . __('Product', 'colissimo-shipping-methods-for-woocommerce') . '</th>';
        $output .= '<th class="td" scope="col" style="text-align:left;">' . __('Quantity', 'colissimo-shipping-methods-for-woocommerce') . '</th>';
        $output .= '<th class="td" scope="col" style="text-align:left;">' . __('Price', 'colissimo-shipping-methods-for-woocommerce') . '</th>';
        $output .= '</tr></thead>';
        ...
        if ($totals) {
            ...
            foreach ($totals as $total) {
                ...
                $output .= '<tr>
						...
						<td class="td" style="text-align:left; ' . ((1 === $i) ? 'border-top-width: 3px;' : '') . '">' . wp_strip_all_tags($total['value']) . '</td>
					</tr>';
            }
        }
        if ($order->get_customer_note()) {
            $output .= '<tr>
					<th class="td" scope="row" colspan="2" style="text-align:left;">' . __('Note', 'colissimo-shipping-methods-for-woocommerce') . '</th>
    ...
    protected function printCustomerInformationInHTML(WC_Order $order) {
        ...
        // Billing address
        $html_output .= '<td><div>';
        $html_output .= '<span style="font-weight: bold;text-decoration: underline;">' . __('Billing address', 'colissimo-shipping-methods-for-woocommerce') . '</span><br>';
        ...
        $html_output .= '<span style="font-weight: bold;text-decoration: underline;">' . __('Shipping address', 'colissimo-shipping-methods-for-woocommerce') . '</span><br>';
    ...
    protected function printStoreInformationInHTML(WC_Order $order) {
        ...
        $output_html = '<div><span style="font-weight: bold;font-size: 1.2em;">' . Helper::get_option('lpc_origin_company_name', '') . '</span><br>';

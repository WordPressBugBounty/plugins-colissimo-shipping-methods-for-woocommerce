...
class Coupons {
    ...
    public function addRestriction($coupon_get_id, $coupon) {
        ...
        $options     = [];
        $options[''] = __('No method', 'colissimo-shipping-methods-for-woocommerce');
        $options     = array_merge($options, Register::get('shippingMethods')->getAllShippingMethods());

        woocommerce_wp_select(
            [
                'id'                => 'lpc_coupon_restriction',
                'name'              => 'lpc_coupon_restriction[]',
                'label'             => __('Exclude shipping methods', 'colissimo-shipping-methods-for-woocommerce'),
                'description'       => __('When the user will enter this coupon code, the following delivery methods won\'t be available.', 'colissimo-shipping-methods-for-woocommerce'),
                'desc_tip'          => true,
                'options'           => $options,

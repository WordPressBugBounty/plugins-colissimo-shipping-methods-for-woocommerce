...
class SettingsTab {
    ...
    protected function initThermalPrintGuide() {
        ...
        add_action('woocommerce_admin_field_lpc_qz_file', [$this, 'displayQzTrayFile']);
        add_action('woocommerce_admin_field_lpc_qz_generate', [$this, 'displayQzTrayGenerate']);
    }
    ...
    public function displayQzTrayGenerate($field) {
        $qzTraySigning = Register::get('qzTraySigning');

        Helper::renderPartial(
            'Settings/QzTrayGenerate.php',
            [
                'title'           => $field['title'],
                'type'            => sanitize_title($field['type']),
                'desc'            => $field['desc'] ?? '',
                'isConfigured'    => $qzTraySigning->isConfigured(),
                'certificateInfo' => $qzTraySigning->getCertificateInfo(),
                'generateUrl'     => $qzTraySigning->getGenerateActionUrl(),
            ]
        );
    }

    public function displayQzTrayFile($field) {
        $qzTraySigning = Register::get('qzTraySigning');
        $isPrivateKey  = QzTraySigning::PRIVATE_KEY_OPTION === $field['id'];

        Helper::renderPartial(
            'Settings/QzTrayFile.php',
            [
                'title'    => $field['title'],
                'type'     => sanitize_title($field['type']),
                'id'       => $field['id'],
                'desc'     => $field['desc'] ?? '',
                'accept'   => $isPrivateKey ? '.pem,.key' : '.txt,.crt,.pem',
                'isFilled' => $isPrivateKey ? '' !== $qzTraySigning->getPrivateKey() : '' !== $qzTraySigning->getCertificate(),
            ]
        );
    }

    /**
     * The QZ Tray certificate and private key are saved here instead of relying on
     * WC_Admin_Settings::save_fields(), which would run wc_clean() on them and destroy the PEM
     * line breaks. Both fields are flagged "is_option": false so WooCommerce leaves them alone.
     *
     * An empty field means "no new file chosen": the saved value is kept.
     */
    protected function saveQzTraySettings(string $currentSection): void {
        if ('label' !== $currentSection) {
            return;
        }

        $qzTraySigning = Register::get('qzTraySigning');
        $certificate   = $this->normalizePem(Helper::getVar(QzTraySigning::CERTIFICATE_OPTION, '', 'string', 'POST'));

        if ('' !== $certificate) {
            if (strlen($certificate) > QzTraySigning::MAX_PEM_LENGTH) {
                WC_Admin_Settings::add_error($this->getQzTrayFileTooLargeError());
                $certificate = '';
            } elseif (false === strpos($certificate, '-----BEGIN CERTIFICATE-----')) {
                WC_Admin_Settings::add_error(
                    __('The QZ Tray certificate is invalid, please choose the digital-certificate.txt file generated by QZ Tray.',
                       'colissimo-shipping-methods-for-woocommerce')
                );
                $certificate = '';
            } else {
                update_option(QzTraySigning::CERTIFICATE_OPTION, $certificate, false);
            }
        }

        $privateKey = $this->normalizePem(Helper::getVar(QzTraySigning::PRIVATE_KEY_OPTION, '', 'string', 'POST'));

        if ('' === $privateKey) {
            return;
        }

        if (strlen($privateKey) > QzTraySigning::MAX_PEM_LENGTH) {
            WC_Admin_Settings::add_error($this->getQzTrayFileTooLargeError());

            return;
        }

        // Certificate chosen along with the key, or the one already saved when only the key is replaced
        if ('' === $certificate) {
            $certificate = $qzTraySigning->getCertificate();
        }

        $error = $qzTraySigning->getPrivateKeyError($privateKey, $certificate);
        ...
        update_option(QzTraySigning::PRIVATE_KEY_OPTION, Helper::encryptPassword($privateKey), false);
    }

    protected function getQzTrayFileTooLargeError(): string {
        return __('The selected file is too large to be a QZ Tray certificate or private key.', 'colissimo-shipping-methods-for-woocommerce');
    }

    /**
     * Files created on Windows carry CRLF line endings, OpenSSL expects a plain PEM content.
     */
    protected function normalizePem(string $content): string {
    ...
    public function settingsSections() {
        ...
        wp_enqueue_script(
            ...
            ['jquery', 'lpc_qz_tray', 'lpc_qz_setup'],
            LPC_VERSION,
            true
        );
        ...
        wp_localize_script(
            ...
            [
                ...
                'detect'            => __('Detect the printers', 'colissimo-shipping-methods-for-woocommerce'),
                'detectHint'        => __('Click "Detect the printers" to list the printers seen by QZ Tray.', 'colissimo-shipping-methods-for-woocommerce'),
                // translators: %s is the name of the selected file.
                'fileSelected'      => __('File selected: %s. Save to apply it.', 'colissimo-shipping-methods-for-woocommerce'),
                'fileError'         => __('This file could not be read.', 'colissimo-shipping-methods-for-woocommerce'),
                'generating'        => __('Generating the certificate…', 'colissimo-shipping-methods-for-woocommerce'),
                'generateError'     => __('The certificate could not be generated.', 'colissimo-shipping-methods-for-woocommerce'),
                'regenerate'        => __('Generate a new certificate', 'colissimo-shipping-methods-for-woocommerce'),
                'regenerateConfirm' => __(
                    'A certificate is already installed. Generating a new one means downloading certificate.crt again and replacing it on every computer that prints.',
                    'colissimo-shipping-methods-for-woocommerce'
                ),
    ...
    public function saveLpcSettings() {
        ...
        try {
            ...
            $this->saveQzTraySettings($currentSection);
        } catch (Exception $exc) {
            Logger::error(

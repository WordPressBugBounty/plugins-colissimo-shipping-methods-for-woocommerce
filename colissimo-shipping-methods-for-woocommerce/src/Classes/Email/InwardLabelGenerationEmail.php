...
class InwardLabelGenerationEmail extends WC_Email {
    public function __construct() {
        $this->id             = 'lpc_inward_label_generation';
        $this->customer_email = true;
        $this->title          = __('Inward label generated', 'colissimo-shipping-methods-for-woocommerce');
        $this->description    = __('An email is sent to the customer when the inward label is generated, if this option is set up', 'colissimo-shipping-methods-for-woocommerce');
    ...
    public function get_default_subject() {
        ...
        return sprintf(__('[%s] Your inward label', 'colissimo-shipping-methods-for-woocommerce'), '{blogname}');
    }

    public function get_default_heading() {
        return __('Inward label generated', 'colissimo-shipping-methods-for-woocommerce');
    }
    ...
    public function trigger(WC_Order $order, $label, $label_filename = 'inward_label') {
        ...
        try {
            ...
            Logger::error('Error sending inward label email', ['error' => $e->getMessage()]);
        } finally {
    ...
    protected function create_attachment($file_name, $content) {
        Helper::getWpFilesystem()->put_contents($file_name, $content, FS_CHMOD_FILE);
    }
    ...
    protected function get_extention_from_format() {
        $extention = Helper::get_option('lpc_returnLabelFormat', 'PDF_A4_300dpi');

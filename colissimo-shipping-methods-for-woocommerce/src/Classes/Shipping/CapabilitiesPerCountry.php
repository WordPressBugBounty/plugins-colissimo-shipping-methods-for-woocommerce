...
class CapabilitiesPerCountry {
    const PATH_TO_COUNTRIES_PER_ZONE_JSON_FILE_FR = LPC_RESOURCE_FOLDER . 'capabilitiesByCountryFR.json';
    const PATH_TO_COUNTRIES_PER_ZONE_JSON_FILE_DOM1 = LPC_RESOURCE_FOLDER . 'capabilitiesByCountryDOM1.json';
    const FROM_FR = 'fr';
    ...
    public function __construct(?ShippingMethods $shippingMethods = null) {
        ...
        register_activation_hook(
            ...
            function () {
                if (is_multisite()) {
                    ...
                    // get_sites() is not available during activation hook execution.
    ...
    public function getProductCodeForOrder(WC_Order $order) {
        ...
        if (true === $productCode) {
            switch ($shippingMethod) {
                case 'lpc_relay':
                    return LabelGenerationPayload::PRODUCT_CODE_RELAY;
                case 'lpc_expert_ddp':
                case 'lpc_expert':
                    return LabelGenerationPayload::PRODUCT_CODE_WITH_SIGNATURE;
                case 'lpc_sign_ddp':
                case 'lpc_sign':
                    if (in_array($countryCode, self::DOM1_COUNTRIES_CODE)) {
                        if ($this->isIntraDOM1($storeCountryCode, $countryCode)) {
                            return LabelGenerationPayload::PRODUCT_CODE_WITH_SIGNATURE_INTRA_DOM;
                        } else {
                            return LabelGenerationPayload::PRODUCT_CODE_WITH_SIGNATURE_OM;
                        }
                    }
                ...
                case 'lpc_nosign':
                    if (in_array($countryCode, self::DOM1_COUNTRIES_CODE)) {
                        if ($this->isIntraDOM1($storeCountryCode, $countryCode)) {
                            return LabelGenerationPayload::PRODUCT_CODE_WITHOUT_SIGNATURE_INTRA_DOM;
                        } else {
                            return LabelGenerationPayload::PRODUCT_CODE_WITHOUT_SIGNATURE_OM;
                        }
                    }
    ...
    public function getReturnProductCodeForDestination($countryCode) {
        ...
        if (true === $returnProductCode) {
            if (in_array($countryCode, self::DOM1_COUNTRIES_CODE) && $this->isIntraDOM1($storeCountryCode, $countryCode)) {
                return LabelGenerationPayload::PRODUCT_CODE_RETURN_FRANCE;
            } else {
                return LabelGenerationPayload::PRODUCT_CODE_RETURN_INT;
            }
        }
    ...
    public function getInfoForDestination($countryCode, $info) {
        ...
        if ('FR' !== $this->getStoreCountryCode() && in_array($info, [SignDdp::ID, ExpertDdp::ID])) {
            return false;
        }
    ...
    protected function getStoreCountryCode($isReturn = false) {
        if ($isReturn) {
            $country = Helper::get_option('lpc_return_address_country', '');
            if (!empty($country)) {
                return $country;
            }
        }

        $country = Helper::get_option('lpc_origin_address_country', '');
        if (!empty($country)) {
            return $country;
        }

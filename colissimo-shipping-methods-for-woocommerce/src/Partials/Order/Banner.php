<?php
// phpcs:disable WordPress.WP.I18n.TextDomainMismatch -- WooCommerce is loaded, using their translations for better accuracy on order page
...
$onDemandServiceLink      = '<a href="' . $args['lpc_ondemand_service_url'] . '" target="_blank">' . __('here', 'colissimo-shipping-methods-for-woocommerce') . '</a>';
...
$multiParcelsLabels       = [
    'MASTER'   => __('Master parcel', 'colissimo-shipping-methods-for-woocommerce'),
    'FOLLOWER' => __('Follower parcel', 'colissimo-shipping-methods-for-woocommerce'),
];
...
<div class="lpc__admin__order_banner">
	<div class="lpc__admin__order_banner__header">
		<div data-lpc-tab="label_listing" class="lpc__admin__order_banner__tab lpc__admin__order_banner__header__listing nav-tab nav-tab-active">
            <?php esc_html_e('Labels listing', 'colissimo-shipping-methods-for-woocommerce'); ?>
		</div>
        <?php if (current_user_can('lpc_manage_labels')) { ?>
			<div data-lpc-tab="generate_label" class="lpc__admin__order_banner__tab lpc__admin__order_banner__header__generation nav-tab">
                <?php esc_html_e('Labels generation', 'colissimo-shipping-methods-for-woocommerce'); ?>
			</div>
        <?php } ?>
        <?php if (current_user_can('lpc_manage_documents') && $customsDocumentsNeeded) { ?>
			<div data-lpc-tab="send_documents" class="lpc__admin__order_banner__tab lpc__admin__order_banner__header__documents nav-tab">
                <?php esc_html_e('Customs documents', 'colissimo-shipping-methods-for-woocommerce'); ?>
			</div>
        <?php } ?>
        <?php if ($collectionAllowed) { ?>
			<div data-lpc-tab="on_demand" class="lpc__admin__order_banner__tab lpc__admin__order_banner__header__ondemand nav-tab">
                <?php esc_html_e('Colissimo collection', 'colissimo-shipping-methods-for-woocommerce'); ?>
			</div>
        <?php } ?>
	</div>
	<div class="lpc__admin__order_banner__content lpc__admin__order_banner__generate_label" style="display: none">
		<div class="lpc__admin__order_banner__generate_label__div">
			<table class="wp-list-table widefat fixed striped">
				<thead>
					<tr>
						...
						<th><?php echo esc_html(sprintf(esc_html__('Unit price (%s)', 'colissimo-shipping-methods-for-woocommerce'), $currency)); ?></th>
						<th><?php esc_html_e('Quantity', 'colissimo-shipping-methods-for-woocommerce'); ?></th>
                        <?php // translators: %s weight unit ?>
						<th><?php echo esc_html(sprintf(esc_html__('Unit weight (%s)', 'colissimo-shipping-methods-for-woocommerce'), $weightUnit)); ?></th>
					</tr>
				</thead>
				<tbody>
                    <?php
                    foreach ($orderItems as $oneItem) {
						...
						<tr>
							<td class="lpc__admin__order_banner__generate_label__item__td__checkbox check-column">
								...
								<input type="checkbox"
								       data-item-id="<?php echo esc_attr($oneItem['id']); ?>"
								       class="lpc__admin__order_banner__generate_label__item__checkbox"
                                    <?php checked(!empty($oneItem['qty'])); ?>
									   name="<?php echo esc_attr($oneItem['id'] . '-checkbox'); ?>"
									   id="<?php echo esc_attr($oneItem['id'] . '-checkbox'); ?>"
								></td>
							<td><?php echo esc_html($oneItem['name']); ?></td>
							<td><input type="number"
							           class="lpc__admin__order_banner__generate_label__item__price"
							           data-item-id="<?php echo esc_attr($oneItem['id']); ?>"
							           value="<?php echo esc_attr($oneItem['price']); ?>"
							           name="<?php echo esc_attr($oneItem['id'] . '-price'); ?>"
							           min="0"
							           step="any"
							           readonly="readonly"
								></td>
							<td><input
										...
										data-item-id="<?php echo esc_attr($oneItem['id']); ?>"
										value="<?php echo esc_attr($oneItem['qty']); ?>"
										step="any"
										min="0"
										name="<?php echo esc_attr($oneItem['id'] . '-qty'); ?>"
										id="<?php echo esc_attr($oneItem['id'] . '-qty'); ?>"
								><span style="margin-left: 5px">/</span><span style="margin-left: 5px"><?php echo esc_html($oneItem['base_qty']); ?></span></td>
							<td><input type="number"
							           class="lpc__admin__order_banner__generate_label__item__weight"
							           data-item-id="<?php echo esc_attr($oneItem['id']); ?>"
							           value="<?php echo esc_attr($oneItem['weight']); ?>"
							           min="0"
							           step="any"
							           readonly="readonly"
							           name="<?php echo esc_attr($oneItem['id'] . '-weight'); ?>"
								></td>
						</tr>
                    <?php } ?>
				</tbody>
			...
			<div class="lpc__admin__order_banner__generate_label__edit_value__container">
                ...
                wp_kses(
                    Helper::tooltip(
                        __('Editing prices and weights may create inconsistency between CN23 or labels and invoice. Edit these values only if you really need it.',
                           'colissimo-shipping-methods-for-woocommerce')
                    ),
                    Helper::KSES_TOOLTIP
                );
                ?>
                <?php esc_html_e('Edit prices and weights', 'colissimo-shipping-methods-for-woocommerce'); ?>
			...
			<div class="lpc__admin__order_banner__generate_label__shipping_costs__container">
				<label for="lpc__admin__order_banner__generate_label__shipping_costs" class="lpc__admin__order_banner_label">
                    <?php // translators: %s currency symbol ?>
                    <?php echo esc_html(sprintf(esc_html__('Shipping costs (%s)', 'colissimo-shipping-methods-for-woocommerce'), $currency)); ?>
				</label>
				<input type="number"
				       min="0"
				       step="any"
				       ...
				       value="<?php echo esc_attr($shippingCosts); ?>"
				       readonly="readonly"
				>
			</div>
            <?php if (!empty($packagings)) { ?>
				<div class="lpc__admin__order_banner__generate_label__packaging__container">
					...
					<label for="lpc__admin__order_banner__generate_label__packaging" class="lpc__admin__order_banner_label">
                        <?php esc_html_e('Packaging', 'colissimo-shipping-methods-for-woocommerce'); ?>
					</label>
					<select
						...
						<option value="auto" id="lpc__admin__order_banner__generate_label__packaging__auto">
                            <?php esc_html_e('Automatic', 'colissimo-shipping-methods-for-woocommerce'); ?>
						</option>
						<option value="custom"><?php esc_html_e('Custom weight', 'colissimo-shipping-methods-for-woocommerce'); ?></option>
                        <?php
			...
			<div class="lpc__admin__order_banner__generate_label__package_weight__container"<?php echo empty($packagings) ? '' : ' style="display: none;"'; ?>>
				<label for="lpc__admin__order_banner__generate_label__package_weight" class="lpc__admin__order_banner_label">
                    <?php // translators: %s weight unit ?>
                    <?php echo esc_html(sprintf(esc_html__('Packaging weight (%s)', 'colissimo-shipping-methods-for-woocommerce'), $weightUnit)); ?>
				</label>
				<input type="number"
				       min="0"
				       step="any"
			...
			<div class="lpc__admin__order_banner__generate_label__total_weight__container">
                <?php esc_html_e('Total weight (items + packaging)', 'colissimo-shipping-methods-for-woocommerce'); ?> :
				<span class="lpc__admin__order_banner__generate_label__total_weight"></span><?php echo esc_html(' ' . $weightUnit); ?>
				<input type="hidden" name="lpc__admin__order_banner__generate_label__weight__unity" value="<?php echo esc_attr($weightUnit); ?>">
            ...
            <?php if ($ddp) { ?>
				<div class="lpc__admin__order_banner__generate_label__package_length__container">
					<label for="lpc__admin__order_banner__generate_label__package_length" class="lpc__admin__order_banner_label">
                        <?php esc_html_e('Package length (cm)', 'colissimo-shipping-methods-for-woocommerce'); ?>
					</label>
					<input type="number"
					       min="0"
					       step="any"
				...
				<div class="lpc__admin__order_banner__generate_label__package_width__container">
					<label for="lpc__admin__order_banner__generate_label__package_width" class="lpc__admin__order_banner_label">
                        <?php esc_html_e('Package width (cm)', 'colissimo-shipping-methods-for-woocommerce'); ?>
					</label>
					<input type="number"
					       min="0"
					       step="any"
				...
				<div class="lpc__admin__order_banner__generate_label__package_height__container">
					<label for="lpc__admin__order_banner__generate_label__package_height" class="lpc__admin__order_banner_label">
                        <?php esc_html_e('Package height (cm)', 'colissimo-shipping-methods-for-woocommerce'); ?>
					</label>
					<input type="number"
					       min="0"
					       step="any"
				...
				<div class="lpc__admin__order_banner__generate_label__package_description__container">
					<label for="lpc__admin__order_banner__generate_label__package_description" class="lpc__admin__order_banner_label">
                        <?php
                        esc_html_e('English description', 'colissimo-shipping-methods-for-woocommerce');
                        wp_kses(
                            Helper::tooltip(__('The customs need a description of the package\'s content, in English.', 'colissimo-shipping-methods-for-woocommerce')),
                            Helper::KSES_TOOLTIP
                        );
                        ?>
					</label>
            ...
            <?php if ($sendingServiceNeeded) { ?>
				<div class="lpc__admin__order_banner__generate_label__sending_service__container">
					<label for="lpc__admin__order_banner__generate_label__sending_service">
                        <?php esc_attr_e('Sending service', 'colissimo-shipping-methods-for-woocommerce'); ?>
					</label>
					<select name="lpc__admin__order_banner__generate_label__sending_service"
						...
						<option value="partner" <?php selected('partner' === $sendingServiceConfig); ?>>
                            <?php esc_attr_e('Local postal service', 'colissimo-shipping-methods-for-woocommerce'); ?>
						</option>
						<option value="dpd" <?php selected('partner' !== $sendingServiceConfig); ?>>DPD</option>
					</select>
				</div>
            <?php } ?>
			<div class="lpc__admin__order_banner__generate_label__non_machinable">
				<label for="lpc__admin__order_banner__generate_label__non_machinable__input">
                    <?php esc_html_e('Non machinable package', 'colissimo-shipping-methods-for-woocommerce'); ?>
				</label>
                <?php if (LabelGenerationPayload::PRODUCT_CODE_RELAY === $productCode) { ?>
                    <?php
                    wp_kses(
                        Helper::tooltip(__('The non-machinable option isn\'t available for this shipping method and destination country.',
                                           'colissimo-shipping-methods-for-woocommerce')),
                        Helper::KSES_TOOLTIP
                    );
                    ?>
				...
				<input
                    ...
                    <?php disabled(LabelGenerationPayload::PRODUCT_CODE_RELAY === $productCode); ?>>
				<p>
                    <?php echo sprintf(
                    // translators: %s documentation link HTML anchor tag
                        esc_html__(
                            'To determine if your package is non machinable you can visit this %s',
                            'colissimo-shipping-methods-for-woocommerce'
                        ),
                        '<a target="_blank" href="https://www.colissimo.entreprise.laposte.fr/fr/expedier">' . esc_html__(
                            'documentation',
                            'colissimo-shipping-methods-for-woocommerce'
                        ) . '</a>'
                    ) ?>
			...
			<div class="lpc__admin__order_banner__generate_label__insurance">
				<label for="lpc_use_insurance">
                    <?php esc_html_e('Use Colissimo Insurance?', 'colissimo-shipping-methods-for-woocommerce'); ?>
				</label>
				<input type="checkbox" <?php checked('yes' === Helper::get_option('lpc_using_insurance')); ?>
			...
			<div class="lpc__admin__order_banner__generate_label__insurance__amount">
				<label for="lpc_insurance_amount">
                    <?php esc_html_e('Personalized amount of insurance:', 'colissimo-shipping-methods-for-woocommerce'); ?>
				</label>
				<select
                    ...
                    <?php disabled('yes' !== Helper::get_option('lpc_using_insurance')); ?>>
					<option value=""><?php esc_html_e('Choose an amount', 'colissimo-shipping-methods-for-woocommerce'); ?></option>
            ...
            <?php if ($cn23Needed) { ?>
				<div class="lpc__admin__order_banner__generate_label__type">
					<label for="lpc_cn23_type">
                        <?php esc_html_e('Customs category', 'colissimo-shipping-methods-for-woocommerce'); ?>
					</label>
					<select
							class="lpc__admin__order_banner__generate_label__cn23__type"
							name="lpc__admin__order_banner__generate_label__cn23__type"
							id="lpc_cn23_type">
						<option value="1" <?php selected(1 === $defaultCustoms); ?>>
                            <?php esc_html_e('Gift', 'colissimo-shipping-methods-for-woocommerce'); ?>
						</option>
						<option value="2" <?php selected(2 === $defaultCustoms); ?>>
                            <?php esc_html_e('Commercial sample', 'colissimo-shipping-methods-for-woocommerce'); ?>
						</option>
						<option value="3" <?php selected(3 === $defaultCustoms); ?>>
                            <?php esc_html_e('Commercial shipment', 'colissimo-shipping-methods-for-woocommerce'); ?>
						</option>
						<option value="4" <?php selected(4 === $defaultCustoms); ?>>
                            <?php esc_html_e('Document', 'colissimo-shipping-methods-for-woocommerce'); ?>
						</option>
						<option value="5" <?php selected(5 === $defaultCustoms); ?>>
                            <?php esc_html_e('Other', 'colissimo-shipping-methods-for-woocommerce'); ?>
						</option>
					</select>
				</div>
            <?php } ?>
            <?php if (1 < $totalQuantity && $isMultiParcelsAuthorized) { ?>
				<div class="lpc__admin__order_banner__generate_label__multi__parcels">
					<label for="lpc_multi_parcels">
                        <?php esc_html_e('Use the multi-parcels shipping', 'colissimo-shipping-methods-for-woocommerce'); ?>
                        <?php
                        wp_kses(
                            Helper::tooltip(
                                __('If you want your client to receive all the parcels at the same time, activate this option and specify the exact number of parcels.',
                                   'colissimo-shipping-methods-for-woocommerce')
                            ),
                            Helper::KSES_TOOLTIP
                        );
					...
					<input type="checkbox"
                        ...
                        <?php checked(!empty($parcelsAmount)); ?>>
				</div>
				<div class="lpc__admin__order_banner__generate_label__multi__parcels__number">
                    <?php if (empty($parcelsAmount)) { ?>
						<label for="lpc_multi_parcels_number">
                            <?php esc_html_e('Number of parcels:', 'colissimo-shipping-methods-for-woocommerce'); ?>
						</label>
						<input type="number"
						       min="2"
                    ...
                    <?php } elseif ($parcelCurrentNumber <= $parcelsAmount) { ?>
                        <?php
                        // translators: %1$s current parcel number, %2$s total parcels amount
                        printf(esc_html__('Parcel n°%1$s / %2$s', 'colissimo-shipping-methods-for-woocommerce'), intval($parcelCurrentNumber), intval($parcelsAmount));
                        wp_kses(
                            Helper::tooltip(
                                __('If you made a mistake or if you want to change the total number of parcels, you can delete generated labels.',
                                   'colissimo-shipping-methods-for-woocommerce')
                            ),
                            Helper::KSES_TOOLTIP
                        );
                        ?>
                    <?php } else { ?>
                        <?php esc_html_e('All labels have been generated.', 'colissimo-shipping-methods-for-woocommerce'); ?>
                    <?php } ?>
				</div>
            <?php } ?>
            <?php if ($args['blocking_code']) { ?>
				<div class="lpc__admin__order_banner__generate_label__block_code">
					<label for="lpc__admin__order_banner__generate_label__block_code__input">
                        <?php esc_html_e('Activate blocking code', 'colissimo-shipping-methods-for-woocommerce'); ?>
					</label>
                    <?php
                    echo wp_kses(
                        wc_help_tip(
			...
			<div class="lpc__admin__order_banner__generate_label__generate-label-button__container">
				<select name="lpc__admin__order_banner__generate_label__outward_or_inward">
					<option value="outward"><?php esc_html_e('Outward label', 'colissimo-shipping-methods-for-woocommerce'); ?></option>
					<option value="inward"<?php disabled($securedReturn); ?>><?php esc_html_e('Inward label', 'colissimo-shipping-methods-for-woocommerce'); ?></option>
					<option value="both"<?php disabled($securedReturn); ?>><?php esc_html_e('Outward and inward labels', 'colissimo-shipping-methods-for-woocommerce'); ?></option>
				</select>
				<button
						type="button"
						class="button button-primary lpc__admin__order_banner__generate_label__generate-label-button"
						data-nonce-param="<?php echo esc_attr(Banner::NONCE_GENERATE_LABEL); ?>"
						data-nonce="<?php echo esc_attr(wp_create_nonce(Banner::NONCE_NAME_GENERATE_LABEL)); ?>">
                    <?php esc_html_e('Generate', 'colissimo-shipping-methods-for-woocommerce'); ?>
				</button>
			</div>
		</div>
	</div>
	<div class="lpc__admin__order_banner__content lpc__admin__order_banner__label_listing">
        <?php if (empty($trackingNumbers)) {
            ...
            echo '<br><div class="lpc__admin__order_banner__warning"><span>' . esc_html($message) . '</span></div>';
        } else { ?>
			<table class="wp-list-table widefat fixed striped">
				<thead>
					<tr>
						<th><?php esc_html_e('Outward labels', 'colissimo-shipping-methods-for-woocommerce'); ?></th>
						<th><?php esc_html_e('Bordereau', 'colissimo-shipping-methods-for-woocommerce'); ?></th>
						<th><?php esc_html_e('Inward labels', 'colissimo-shipping-methods-for-woocommerce'); ?></th>
					</tr>
				</thead>
						...
						<tr>
							<td>
                                ...
                                if ('no_outward' !== $outwardTrackingNumber) {
									...
									<a target="_blank" href="<?php echo esc_url($trackingLink); ?>">
                                        <?php
                                        echo esc_html($outwardTrackingNumber);

                                        $additionalInformation = [];
                                        if (in_array($outwardTrackingNumber, $insured)) {
                                            $additionalInformation[] = __('Insured', 'colissimo-shipping-methods-for-woocommerce');
                                        }
                                    ...
                                    echo '<br />' . esc_html(__('Creation date',
                                                                'colissimo-shipping-methods-for-woocommerce') . ': ' . (new WC_DateTime($label->label_created_at))->date_i18n($dateFormat));
                                    if (!empty($label->status_id)) {
                                        $labelStatus = $args['colissimoStatus']->getStatusInfo($label->status_id)['label'];
                                        echo '<br />' . esc_html(__('Status', 'colissimo-shipping-methods-for-woocommerce') . ': ' . $labelStatus);
                                    }
                                    ?>
									<br>
                                    <?php echo wp_kses_post(
                                        $args['lpc_label_queries']->getOutwardLabelsActionsIcons(
							...
							<td>
                                ...
                                if ('no_outward' !== $outwardTrackingNumber) {
                                    if (!empty($bordereauLinks[$outwardTrackingNumber])) {
                                        // translators: %d bordereau id number
                                        esc_html(sprintf(__('Bordereau n°%d', 'colissimo-shipping-methods-for-woocommerce'), $bordereauLinks[$outwardTrackingNumber]['id']));
                                        echo '<br>';
                                        echo wp_kses_post(
                                            $args['lpc_bordereau_queries']->getBordereauActionsIcons(
							...
							<td>
                                <?php foreach ($inwardTrackingNumbers as $inwardTrackingNumber) { ?>
                                    <?php echo esc_html($inwardTrackingNumber); ?>
									<br>
                                    <?php
                                    echo wp_kses_post(
                                        $args['lpc_label_queries']->getInwardLabelsActionsIcons(
	...
	<div class="lpc__admin__order_banner__content lpc__admin__order_banner__send_documents" style="display: none">
					...
					<select class="lpc__admin__order_banner__document__type">
                        ...
                        if (!empty($args['lpc_documents_types'])) {
                            foreach ($args['lpc_documents_types'] as $documentType => $description) {
                                echo '<option value="' . esc_attr($documentType) . '">' . esc_html($description) . '</option>';
                            }
                        }
                        ?>
					</select>
		...
		<table
				...
				data-nonce-param="<?php echo esc_attr(Banner::NONCE_SEND_DOCUMENTS); ?>"
				data-nonce="<?php echo esc_attr(wp_create_nonce(Banner::NONCE_NAME_SEND_DOCUMENTS)); ?>">
			<thead>
                <?php foreach ($trackingNumbers as $outwardTrackingNumber => $inwardTrackingNumbers) { ?>
					<tr>
						<th><?php echo esc_html($outwardTrackingNumber); ?></th>
						<td class="lpc__admin__order_banner__send_documents__container">
                                    ...
                                    if (!empty($args['lpc_sent_documents'][$outwardTrackingNumber])) {
                                        foreach ($args['lpc_sent_documents'][$outwardTrackingNumber] as $oneDocument) {
											...
											<tr class="lpc__customs__sent__document">
												<td>
                                                    <?php echo esc_html($args['lpc_documents_types'][$oneDocument['documentType']]); ?>
												</td>
												<td>
                                                    <?php echo esc_html($oneDocument['documentName']); ?>
												</td>
											</tr>
                                            <?php
                                        }
							...
							<div class="text-center">
								<button type="button"
								        ...
								        data-lpc-parcelnumber="<?php echo esc_attr($outwardTrackingNumber); ?>">
                                    <?php esc_html_e('Add an other document', 'colissimo-shipping-methods-for-woocommerce'); ?>
								</button>
								<button type="button" class="button button-primary lpc__admin__order_banner__send_documents__listing__send_button">
                                    <?php esc_html_e('Submit the documents', 'colissimo-shipping-methods-for-woocommerce'); ?>
								</button>
							</div>
						</td>
					</tr>
		...
		<div style="margin-top: 1rem;">
            <?php esc_html_e('In accordance with the customs regulation, it is necessary to provide documents related to the parcels for the customs.',
                             'colissimo-shipping-methods-for-woocommerce'); ?>
			<br />
            <?php esc_html_e('It is possible to send these documents through the parcel tracking tool or from here using the plugin.',
                             'colissimo-shipping-methods-for-woocommerce'); ?>
		</div>
	</div>
	<div class="lpc__admin__order_banner__content lpc__admin__order_banner__on_demand" style="display: none">
		...
		<div>
            <?php
            printf(
            // translators: %s account link HTML anchor tag
                esc_html__(
                    'To benefit from the collection, connect to your account %s and access the collection section to subscribe to the service.',
                    'colissimo-shipping-methods-for-woocommerce'
                ),
                wp_kses(
                    $onDemandServiceLink,
        ...
        <?php // translators: %s Mac download link HTML anchor tag ?>
		<div><?php printf(esc_html__('Here is the link for %s', 'colissimo-shipping-methods-for-woocommerce'), wp_kses($onDemandMacLink, Helper::KSES_LINK)); ?></div>
        <?php // translators: %s Windows download link HTML anchor tag ?>
		<div><?php printf(esc_html__('Here is the link for %s', 'colissimo-shipping-methods-for-woocommerce'), wp_kses($onDemandWindowsLink, Helper::KSES_LINK)); ?></div>
		<br />
		<div><?php esc_html_e('To benefit from on-demand collection, you must have subscribed to the service from your Colissimo space.',
		...
		<div>
			<ol>
				<li><?php esc_html_e('If you are a Facilité customer, go to your Colissimo space in the Collection section and make a subscription request.',
                                     'colissimo-shipping-methods-for-woocommerce'); ?></li>
				<li><?php esc_html_e('If you are a Privilege customer, contact your usual sales contact so that he can give you access to the section.',
                                     'colissimo-shipping-methods-for-woocommerce'); ?></li>
			</ol>
		</div>
	</div>

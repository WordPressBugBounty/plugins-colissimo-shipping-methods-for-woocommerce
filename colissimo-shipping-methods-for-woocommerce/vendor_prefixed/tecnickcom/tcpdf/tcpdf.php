...
//
// This file is part of ColissimoTCPDF software library.
//
// ColissimoTCPDF is free software: you can redistribute it and/or modify it
// under the terms of the GNU Lesser General Public License as
// published by the Free Software Foundation, either version 3 of the
// License, or (at your option) any later version.
//
// ColissimoTCPDF is distributed in the hope that it will be useful, but
// WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
// See the GNU Lesser General Public License for more details.
//
// You should have received a copy of the License
// along with ColissimoTCPDF. If not, see
...
//  * 1D and 2D barcodes: CODE 39, ANSI MH10.8M-1983, USD-3, 3 of 9, CODE 93, USS-93, Standard 2 of 5, Interleaved 2 of 5, CODE 128 A/B/C, 2 and 5 Digits UPC-Based Extension, EAN 8, EAN 13, UPC-A, UPC-E, MSI, POSTNET, PLANET, RMS4CC (Royal Mail 4-state Customer Code), CBC (Customer Bar Code), KIX (Klant index - Customer index), Intelligent Mail Barcode, Onecode, USPS-B-3200, CODABAR, CODE 11, PHARMACODE, PHARMACODE TWO-TRACKS, ColissimoDatamatrix, QR-Code, ColissimoPDF417;
 ...
 * ColissimoTCPDF project (http://www.tcpdf.org) was originally derived in 2002 from the Public Domain FPDF class by Olivier Plathey (http://www.fpdf.org), but now is almost entirely rewritten.<br>
 * <h3>ColissimoTCPDF main features are:</h3>
 * <ul>
...
// ColissimoTCPDF configuration
require_once(dirname(__FILE__).'/tcpdf_autoconfig.php');
// ColissimoTCPDF static font methods and data
require_once(dirname(__FILE__).'/include/tcpdf_font_data.php');
// ColissimoTCPDF static font methods and data
require_once(dirname(__FILE__).'/include/tcpdf_fonts.php');
// ColissimoTCPDF static color methods and data
require_once(dirname(__FILE__).'/include/tcpdf_colors.php');
// ColissimoTCPDF static image methods and data
require_once(dirname(__FILE__).'/include/tcpdf_images.php');
// ColissimoTCPDF static methods and data
require_once(dirname(__FILE__).'/include/tcpdf_static.php');

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

/**
 * @class ColissimoTCPDF
 * PHP class for generating PDF documents without requiring external extensions.
 * ColissimoTCPDF project (http://www.tcpdf.org) has been originally derived in 2002 from the Public Domain FPDF class by Olivier Plathey (http://www.fpdf.org), but now is almost entirely rewritten.<br>
 * @package com.tecnick.tcpdf
...
class ColissimoTCPDF {
	...
	/**
	 * If true print ColissimoTCPDF meta link.
	 * @protected
	 * @since 5.9.152 (2012-03-23)
	 */
	...
	public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false, $pdfa=false) {
		...
		$this->file_id = md5(ColissimoTCPDF_STATIC::getRandomSeed('ColissimoTCPDF'.$orientation.$unit.$serformat.$encoding));
		$this->hash_key = hash_hmac('sha256', ColissimoTCPDF_STATIC::getRandomSeed($this->file_id), ColissimoTCPDF_STATIC::getRandomSeed('ColissimoTCPDF'), false);
		...
		ColissimoTCPDF_FONTS::utf8Bidi(array(), '', false, $this->isunicode, $this->CurrentFont);
		// set default font
	...
	protected function setPageFormat($format, $orientation='P') {
		...
		if (is_string($format)) {
			...
			$pf = ColissimoTCPDF_STATIC::getPageSizeFromFormat($format);
			$this->fwPt = $pf[0];
			$this->fhPt = $pf[1];
		} else {
			// the boundaries of the physical medium on which the page shall be displayed or printed
			if (isset($format['MediaBox'])) {
				$this->pagedim = ColissimoTCPDF_STATIC::setPageBoxes($this->page, 'MediaBox', $format['MediaBox']['llx'], $format['MediaBox']['lly'], $format['MediaBox']['urx'], $format['MediaBox']['ury'], false, $this->k, $this->pagedim);
				...
				if (isset($format[0]) AND is_numeric($format[0]) AND isset($format[1]) AND is_numeric($format[1])) {
					...
					$pf = ColissimoTCPDF_STATIC::getPageSizeFromFormat($format['format']);
				}
				$this->fwPt = $pf[0];
				$this->fhPt = $pf[1];
				$this->pagedim = ColissimoTCPDF_STATIC::setPageBoxes($this->page, 'MediaBox', 0, 0, $this->fwPt, $this->fhPt, true, $this->k, $this->pagedim);
			}
			// the visible region of default user space
			if (isset($format['CropBox'])) {
				$this->pagedim = ColissimoTCPDF_STATIC::setPageBoxes($this->page, 'CropBox', $format['CropBox']['llx'], $format['CropBox']['lly'], $format['CropBox']['urx'], $format['CropBox']['ury'], false, $this->k, $this->pagedim);
			}
			// the region to which the contents of the page shall be clipped when output in a production environment
			if (isset($format['BleedBox'])) {
				$this->pagedim = ColissimoTCPDF_STATIC::setPageBoxes($this->page, 'BleedBox', $format['BleedBox']['llx'], $format['BleedBox']['lly'], $format['BleedBox']['urx'], $format['BleedBox']['ury'], false, $this->k, $this->pagedim);
			}
			// the intended dimensions of the finished page after trimming
			if (isset($format['TrimBox'])) {
				$this->pagedim = ColissimoTCPDF_STATIC::setPageBoxes($this->page, 'TrimBox', $format['TrimBox']['llx'], $format['TrimBox']['lly'], $format['TrimBox']['urx'], $format['TrimBox']['ury'], false, $this->k, $this->pagedim);
			}
			// the page's meaningful content (including potential white space)
			if (isset($format['ArtBox'])) {
				$this->pagedim = ColissimoTCPDF_STATIC::setPageBoxes($this->page, 'ArtBox', $format['ArtBox']['llx'], $format['ArtBox']['lly'], $format['ArtBox']['urx'], $format['ArtBox']['ury'], false, $this->k, $this->pagedim);
			}
	...
	public function setPageOrientation($orientation, $autopagebreak=null, $bottommargin=null) {
		...
		if (!isset($this->pagedim[$this->page]['CropBox'])) {
			// the visible region of default user space
			$this->pagedim = ColissimoTCPDF_STATIC::setPageBoxes($this->page, 'CropBox', $this->pagedim[$this->page]['MediaBox']['llx'], $this->pagedim[$this->page]['MediaBox']['lly'], $this->pagedim[$this->page]['MediaBox']['urx'], $this->pagedim[$this->page]['MediaBox']['ury'], true, $this->k, $this->pagedim);
		}
		if (!isset($this->pagedim[$this->page]['BleedBox'])) {
			// the region to which the contents of the page shall be clipped when output in a production environment
			$this->pagedim = ColissimoTCPDF_STATIC::setPageBoxes($this->page, 'BleedBox', $this->pagedim[$this->page]['CropBox']['llx'], $this->pagedim[$this->page]['CropBox']['lly'], $this->pagedim[$this->page]['CropBox']['urx'], $this->pagedim[$this->page]['CropBox']['ury'], true, $this->k, $this->pagedim);
		}
		if (!isset($this->pagedim[$this->page]['TrimBox'])) {
			// the intended dimensions of the finished page after trimming
			$this->pagedim = ColissimoTCPDF_STATIC::setPageBoxes($this->page, 'TrimBox', $this->pagedim[$this->page]['CropBox']['llx'], $this->pagedim[$this->page]['CropBox']['lly'], $this->pagedim[$this->page]['CropBox']['urx'], $this->pagedim[$this->page]['CropBox']['ury'], true, $this->k, $this->pagedim);
		}
		if (!isset($this->pagedim[$this->page]['ArtBox'])) {
			// the page's meaningful content (including potential white space)
			$this->pagedim = ColissimoTCPDF_STATIC::setPageBoxes($this->page, 'ArtBox', $this->pagedim[$this->page]['CropBox']['llx'], $this->pagedim[$this->page]['CropBox']['lly'], $this->pagedim[$this->page]['CropBox']['urx'], $this->pagedim[$this->page]['CropBox']['ury'], true, $this->k, $this->pagedim);
		}
		...
		if ((abs($this->pagedim[$this->page]['MediaBox']['urx'] - $this->hPt) < $this->feps) AND (abs($this->pagedim[$this->page]['MediaBox']['ury'] - $this->wPt) < $this->feps)){
			...
			$this->pagedim = ColissimoTCPDF_STATIC::swapPageBoxCoordinates($this->page, $this->pagedim);
		}
		$this->w = ($this->wPt / $this->k);
		$this->h = ($this->hPt / $this->k);
		if (ColissimoTCPDF_STATIC::empty_string($autopagebreak)) {
		...
		if (ColissimoTCPDF_STATIC::empty_string($bottommargin)) {
	...
	public function setCellPaddings($left=null, $top=null, $right=null, $bottom=null) {
		if (!ColissimoTCPDF_STATIC::empty_string($left) AND ($left >= 0)) {
			$this->cell_padding['L'] = $left;
		}
		if (!ColissimoTCPDF_STATIC::empty_string($top) AND ($top >= 0)) {
			$this->cell_padding['T'] = $top;
		}
		if (!ColissimoTCPDF_STATIC::empty_string($right) AND ($right >= 0)) {
			$this->cell_padding['R'] = $right;
		}
		if (!ColissimoTCPDF_STATIC::empty_string($bottom) AND ($bottom >= 0)) {
	...
	public function setDisplayMode($zoom, $layout='SinglePage', $mode='UseNone') {
		...
		$this->LayoutMode = ColissimoTCPDF_STATIC::getPageLayoutMode($layout);
		$this->PageMode = ColissimoTCPDF_STATIC::getPageMode($mode);
	}
	...
	public function Error($msg) {
		...
		if (defined('K_TCPDF_THROW_EXCEPTION_ERROR') AND !K_TCPDF_THROW_EXCEPTION_ERROR) {
			die('<strong>ColissimoTCPDF ERROR: </strong>'.$msg);
		} else {
			throw new Exception('ColissimoTCPDF ERROR: '.$msg);
		}
	}
	...
	public function Header() {
		if ($this->header_xobjid === false) {
			...
			if (($headerdata['logo']) AND ($headerdata['logo'] != K_BLANK_IMAGE)) {
				$imgtype = ColissimoTCPDF_IMAGES::getImageFileType(K_PATH_IMAGES.$headerdata['logo']);
	...
	protected function setTableHeader() {
		...
		if (!ColissimoTCPDF_STATIC::empty_string($this->thead) AND (!$this->inthead)) {
			// set margins
	...
	public function setSpotColor($type, $name, $tint=100) {
		$spotcolor = ColissimoTCPDF_COLORS::getSpotColor($name, $this->spot_colors);
		if ($spotcolor === false) {
	...
	public function GetStringWidth($s, $fontname='', $fontstyle='', $fontsize=0, $getarray=false) {
		return $this->GetArrStringWidth(ColissimoTCPDF_FONTS::utf8Bidi(ColissimoTCPDF_FONTS::UTF8StringToArray($s, $this->isunicode, $this->CurrentFont), $s, $this->tmprtl, $this->isunicode, $this->CurrentFont), $fontname, $fontstyle, $fontsize, $getarray);
	}
	...
	public function GetArrStringWidth($sa, $fontname='', $fontstyle='', $fontsize=0, $getarray=false) {
		...
		if (!ColissimoTCPDF_STATIC::empty_string($fontname)) {
		...
		if ($this->isunicode AND (!$this->isUnicodeFont())) {
			$sa = ColissimoTCPDF_FONTS::UTF8ArrToLatin1Arr($sa);
		}
		$w = 0; // total width
	...
	public function GetNumChars($s) {
		if ($this->isUnicodeFont()) {
			return count(ColissimoTCPDF_FONTS::UTF8StringToArray($s, $this->isunicode, $this->CurrentFont));
		}
		return strlen($s);
	}
	...
	protected function getFontsList() {
		if (($fontsdir = opendir(ColissimoTCPDF_FONTS::_getfontpath())) !== false) {
	...
	public function AddFont($family, $style='', $fontfile='', $subset='default') {
		...
		if (ColissimoTCPDF_STATIC::empty_string($family)) {
			if (!ColissimoTCPDF_STATIC::empty_string($this->FontFamily)) {
		...
		$fontdir = false;
		if (!ColissimoTCPDF_STATIC::empty_string($fontfile)) {
			$fontdir = dirname($fontfile);
			if (ColissimoTCPDF_STATIC::empty_string($fontdir) OR ($fontdir == '.')) {
				$fontdir = '';
			} else {
				$fontdir .= '/';
			}
		...
		if (ColissimoTCPDF_STATIC::empty_string($fontfile) OR (!@ColissimoTCPDF_STATIC::file_exists($fontfile))) {
			// build a standard filenames for specified font
			$tmp_fontfile = str_replace(' ', '', $family).strtolower($style).'.php';
			$fontfile = ColissimoTCPDF_FONTS::getFontFullPath($tmp_fontfile, $fontdir);
			if (ColissimoTCPDF_STATIC::empty_string($fontfile)) {
				$missing_style = true;
		...
		// include font file
		if (!ColissimoTCPDF_STATIC::empty_string($fontfile) AND (@ColissimoTCPDF_STATIC::file_exists($fontfile))) {
			$type=null;
			$name=null;
			$desc=null;
			$up=-null;
		...
		if (!isset($file) OR ColissimoTCPDF_STATIC::empty_string($file)) {
			$file = '';
		}
		if (!isset($enc) OR ColissimoTCPDF_STATIC::empty_string($enc)) {
			$enc = '';
		}
		if (!isset($cidinfo) OR ColissimoTCPDF_STATIC::empty_string($cidinfo)) {
			$cidinfo = array('Registry'=>'Adobe', 'Ordering'=>'Identity', 'Supplement'=>0);
			$cidinfo['uni2cid'] = array();
		}
		if (!isset($ctg) OR ColissimoTCPDF_STATIC::empty_string($ctg)) {
			$ctg = '';
		}
		if (!isset($desc) OR ColissimoTCPDF_STATIC::empty_string($desc)) {
			$desc = array();
		}
		if (!isset($up) OR ColissimoTCPDF_STATIC::empty_string($up)) {
			$up = -100;
		}
		if (!isset($ut) OR ColissimoTCPDF_STATIC::empty_string($ut)) {
			$ut = 50;
		}
		if (!isset($cw) OR ColissimoTCPDF_STATIC::empty_string($cw)) {
			$cw = array();
		}
		if (!isset($dw) OR ColissimoTCPDF_STATIC::empty_string($dw)) {
			// set default width
		...
		if (!ColissimoTCPDF_STATIC::empty_string($file)) {
	...
	public function isCharDefined($char, $font='', $style='') {
		if (is_string($char)) {
			// get character code
			$char = ColissimoTCPDF_FONTS::UTF8StringToArray($char, $this->isunicode, $this->CurrentFont);
			$char = $char[0];
		}
		if (ColissimoTCPDF_STATIC::empty_string($font)) {
			if (ColissimoTCPDF_STATIC::empty_string($style)) {
	...
	public function replaceMissingChars($text, $font='', $style='', $subs=array()) {
		...
		$fontdata = $this->AddFont($font, $style);
		$fontinfo = $this->getFontBuffer($fontdata['fontkey']);
		$uniarr = ColissimoTCPDF_FONTS::UTF8StringToArray($text, $this->isunicode, $this->CurrentFont);
		...
		return ColissimoTCPDF_FONTS::UniArrSubString(ColissimoTCPDF_FONTS::UTF8ArrayToUniArray($uniarr, $this->isunicode));
	}
	...
	public function Annotation($x, $y, $w, $h, $text, $opt=array('Subtype'=>'Text'), $spaces=0) {
		...
		if (!$this->pdfa_mode || ($this->pdfa_mode && $this->pdfa_version == 3)) {
			if ((($opt['Subtype'] == 'FileAttachment') OR ($opt['Subtype'] == 'Sound')) AND (!ColissimoTCPDF_STATIC::empty_string($opt['FS']))
				AND (@ColissimoTCPDF_STATIC::file_exists($opt['FS']) OR ColissimoTCPDF_STATIC::isValidURL($opt['FS']))
				AND (!isset($this->embeddedfiles[basename($opt['FS'])]))) {
				$this->embeddedfiles[basename($opt['FS'])] = array('f' => ++$this->n, 'n' => ++$this->n, 'file' => $opt['FS']);
			}
		}
		// Add widgets annotation's icons
		if (isset($opt['mk']['i']) AND @ColissimoTCPDF_STATIC::file_exists($opt['mk']['i'])) {
			$this->Image($opt['mk']['i'], '', '', 10, 10, '', '', '', false, 300, '', false, false, 0, false, true);
		}
		if (isset($opt['mk']['ri']) AND @ColissimoTCPDF_STATIC::file_exists($opt['mk']['ri'])) {
			$this->Image($opt['mk']['ri'], '', '', 0, 0, '', '', '', false, 300, '', false, false, 0, false, true);
		}
		if (isset($opt['mk']['ix']) AND @ColissimoTCPDF_STATIC::file_exists($opt['mk']['ix'])) {
	...
	public function EmbedFile($opt) {
		if (!$this->pdfa_mode || ($this->pdfa_mode && $this->pdfa_version == 3)) {
			if ((($opt['Subtype'] == 'FileAttachment')) AND (!ColissimoTCPDF_STATIC::empty_string($opt['FS']))
				AND (@ColissimoTCPDF_STATIC::file_exists($opt['FS']) OR ColissimoTCPDF_STATIC::isValidURL($opt['FS']))
				AND (!isset($this->embeddedfiles[basename($opt['FS'])]))) {
				$this->embeddedfiles[basename($opt['FS'])] = array('f' => ++$this->n, 'n' => ++$this->n, 'file' => $opt['FS']);
			}
	...
	protected function checkPageBreak($h=0, $y=null, $addpage=true) {
		if (ColissimoTCPDF_STATIC::empty_string($y)) {
			$y = $this->y;
		}
	...
	protected function getCellCode($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M') {
		...
		$txt = is_null($txt) ? '' : $txt;
		$txt = str_replace(ColissimoTCPDF_FONTS::unichr(160, $this->isunicode), ' ', $txt);
		$prev_cell_margin = $this->cell_margin;
		$prev_cell_padding = $this->cell_padding;
		$txt = ColissimoTCPDF_STATIC::removeSHY($txt, $this->isunicode);
		...
		if (ColissimoTCPDF_STATIC::empty_string($w) OR ($w <= 0)) {
			if ($this->rtl) {
				$w = $x - $this->lMargin;
			} else {
		...
		if ($txt != '') {
			...
			if ($this->isunicode) {
				if (($this->CurrentFont['type'] == 'core') OR ($this->CurrentFont['type'] == 'TrueType') OR ($this->CurrentFont['type'] == 'Type1')) {
					$txt2 = ColissimoTCPDF_FONTS::UTF8ToLatin1($txt2, $this->isunicode, $this->CurrentFont);
				} else {
					$unicode = ColissimoTCPDF_FONTS::UTF8StringToArray($txt, $this->isunicode, $this->CurrentFont); // array of UTF-8 unicode values
					$unicode = ColissimoTCPDF_FONTS::utf8Bidi($unicode, '', $this->tmprtl, $this->isunicode, $this->CurrentFont);
					...
					$txt2 = ColissimoTCPDF_FONTS::arrUTF8ToUTF16BE($unicode, false);
				}
			}
			$txt2 = ColissimoTCPDF_STATIC::_escape($txt2);
			...
			if (isset($uniblock)) { // @phpstan-ignore-line
				...
				foreach ($uniblock as $uk => $uniarr) { // @phpstan-ignore-line
					if (($uk % 2) == 0) {
						...
						// character to print
						$topchr = ColissimoTCPDF_FONTS::arrUTF8ToUTF16BE($uniarr, false);
						$topchr = ColissimoTCPDF_STATIC::_escape($topchr);
	...
	public function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=false, $ln=1, $x=null, $y=null, $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0, $valign='T', $fitcell=false) {
		...
		$this->setCellMargins(0, 0, 0, 0);
		if (ColissimoTCPDF_STATIC::empty_string($this->lasth) OR $reseth) {
			// reset row height
			$this->resetLastH();
		}
		if (!ColissimoTCPDF_STATIC::empty_string($y)) {
		...
		if (!ColissimoTCPDF_STATIC::empty_string($x)) {
			$this->setX($x);
		} else {
			$x = $this->GetX();
		}
		...
		// get border modes
		$border_start = ColissimoTCPDF_STATIC::getBorderMode($border, $position='start', $this->opencell);
		$border_end = ColissimoTCPDF_STATIC::getBorderMode($border, $position='end', $this->opencell);
		$border_middle = ColissimoTCPDF_STATIC::getBorderMode($border, $position='middle', $this->opencell);
	...
	public function getNumLines($txt, $w=0, $reseth=false, $autopadding=true, $cellpadding=null, $border=0) {
		...
		$lines = 1;
		$sum = 0;
		$chars = ColissimoTCPDF_FONTS::utf8Bidi(ColissimoTCPDF_FONTS::UTF8StringToArray($txt, $this->isunicode, $this->CurrentFont), $txt, $this->tmprtl, $this->isunicode, $this->CurrentFont);
		...
		for ($i = 0; $i < $length; ++$i) {
			...
			if (($c != 160)
					AND (($c == 173)
						OR preg_match($this->re_spaces, ColissimoTCPDF_FONTS::unichr($c, $this->isunicode))
						OR (($c == 45)
							AND ($i > 0) AND ($i < ($length - 1))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], ColissimoTCPDF_FONTS::unichr($chars[($i - 1)], $this->isunicode))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], ColissimoTCPDF_FONTS::unichr($chars[($i + 1)], $this->isunicode))
						)
					)
				) {
	...
	public function Write($h, $txt, $link='', $fill=false, $align='', $ln=false, $stretch=0, $firstline=false, $firstblock=false, $maxh=0, $wadj=0, $margin=null) {
		...
		if (preg_match(ColissimoTCPDF_FONT_DATA::$uni_RE_PATTERN_ARABIC, $s)) {
			$arabic = true;
		} else {
			$arabic = false;
		}
		// check if string contains RTL text
		if ($arabic OR ($this->tmprtl == 'R') OR preg_match(ColissimoTCPDF_FONT_DATA::$uni_RE_PATTERN_RTL, $s)) {
			$rtlmode = true;
		} else {
			$rtlmode = false;
		}
		...
		$chars = ColissimoTCPDF_FONTS::UTF8StringToArray($s, $this->isunicode, $this->CurrentFont);
		...
		// get array of chars
		$uchars = ColissimoTCPDF_FONTS::UTF8ArrayToUniArray($chars, $this->isunicode);
		// get the number of characters
		$nb = count($chars);
		// replacement for SHY character (minus symbol)
		$shy_replacement = 45;
		$shy_replacement_char = ColissimoTCPDF_FONTS::unichr($shy_replacement, $this->isunicode);
		...
		while ($i < $nb) {
			...
			if ($c == 10) { // 10 = "\n" = new line
				...
				$tmpstr = ColissimoTCPDF_FONTS::UniArrSubString($uchars, $j, $i);
				if ($firstline) {
					$startx = $this->x;
					$tmparr = array_slice($chars, $j, ($i - $j));
					if ($rtlmode) {
						$tmparr = ColissimoTCPDF_FONTS::utf8Bidi($tmparr, $tmpstr, $this->tmprtl, $this->isunicode, $this->CurrentFont);
					}
				...
				if ($firstline) {
					...
					return (ColissimoTCPDF_FONTS::UniArrSubString($uchars, $i));
				}
				++$nl;
				$j = $i + 1;
				$l = 0;
				...
				if (($c != 160)
					AND (($c == 173)
						...
						OR (($c == 45)
							AND ($i < ($nb - 1))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], ColissimoTCPDF_FONTS::unichr($pc, $this->isunicode))
							AND @preg_match('/[\p{L}]/'.$this->re_space['m'], ColissimoTCPDF_FONTS::unichr($chars[($i + 1)], $this->isunicode))
						)
					)
				) {
				...
				if ($this->isUnicodeFont() AND ($arabic)) {
					...
					// *** very slow ***
					$l = $this->GetArrStringWidth(ColissimoTCPDF_FONTS::utf8Bidi(array_slice($chars, $j, ($i - $j)), '', $this->tmprtl, $this->isunicode, $this->CurrentFont));
				} else {
				...
				if (($l > $wmax) OR (($c == 173) AND (($l + $tmp_shy_replacement_width) >= $wmax))) {
					...
					if ($sep == -1) {
						...
						if (($this->rtl AND ($this->x <= ($this->w - $this->rMargin - $this->cell_padding['R'] - $margin['R'] - $chrwidth)))
							...
							$linebreak = true;
							if ($firstline) {
								return (ColissimoTCPDF_FONTS::UniArrSubString($uchars, $j));
							}
						} else {
						...
						$strrest = ColissimoTCPDF_FONTS::UniArrSubString($uchars, ($sep + $endspace));
						$nextstr = ColissimoTCPDF_STATIC::pregSplit('/'.$this->re_space['p'].'/', $this->re_space['m'], $this->stringTrim($strrest));
						if (isset($nextstr[0]) AND ($this->GetStringWidth($nextstr[0]) > $pw)) {
							...
							$tmpstr = ColissimoTCPDF_FONTS::UniArrSubString($uchars, $j, ($sep + $endspace));
							if ($firstline) {
								$startx = $this->x;
							...
							if ($firstline) {
								...
								$this->cell_padding = $tmpcellpadding;
								return (ColissimoTCPDF_FONTS::UniArrSubString($uchars, ($sep + $endspace)));
							}
							$i = $sep;
							$sep = -1;
							$shy = false;
		...
		if ($l > 0) {
			...
			$tmpstr = ColissimoTCPDF_FONTS::UniArrSubString($uchars, $j, $nb);
			if ($firstline) {
				$startx = $this->x;
			...
			if ($firstline) {
				$this->cell_padding = $tmpcellpadding;
				return (ColissimoTCPDF_FONTS::UniArrSubString($uchars, $nb));
			}
			++$nl;
		}
		if ($firstline) {
	...
	public function Image($file, $x=null, $y=null, $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		if (!empty($imgdata)) {
			// copy image to cache
			$original_file = $file;
			$file = ColissimoTCPDF_STATIC::getObjFilename('img', $this->file_id);
			$fp = ColissimoTCPDF_STATIC::fopenLocal($file, 'w');
			if (!$fp) {
		...
		if ($newimage) {
			...
			if ($type == '') {
				$type = ColissimoTCPDF_IMAGES::getImageFileType($file, $imsize);
			} elseif ($type == 'jpg') {
				$type = 'jpeg';
			}
			// Specific image handlers (defined on ColissimoTCPDF_IMAGES CLASS)
			$mtd = '_parse'.$type;
			// GD image handler function
			$gdfunction = 'imagecreatefrom'.$type;
			$info = false;
			if ((method_exists('ColissimoTCPDF_IMAGES', $mtd)) AND (!($resize AND (function_exists($gdfunction) OR extension_loaded('imagick'))))) {
				// ColissimoTCPDF image functions
				$info = ColissimoTCPDF_IMAGES::$mtd($file);
					...
					if ($img !== false) {
						if ($resize) {
							...
							if (($type == 'gif') OR ($type == 'png')) {
								$imgr = ColissimoTCPDF_IMAGES::setGDImageTransparency($imgr, $img);
							}
							imagecopyresampled($imgr, $img, 0, 0, 0, 0, $neww, $newh, $pixw, $pixh);
							$img = $imgr;
						}
						if (($type == 'gif') OR ($type == 'png')) {
							$info = ColissimoTCPDF_IMAGES::_toPNG($img, ColissimoTCPDF_STATIC::getObjFilename('img', $this->file_id));
						} else {
							$info = ColissimoTCPDF_IMAGES::_toJPEG($img, $this->jpeg_quality, ColissimoTCPDF_STATIC::getObjFilename('img', $this->file_id));
						}
					}
				} catch(Exception $e) {
					$info = false;
			...
			if (($info === false) AND extension_loaded('imagick')) {
				try {
					...
					$tempname = ColissimoTCPDF_STATIC::getObjFilename('img', $this->file_id);
					$img->writeImage($tempname);
					$info = ColissimoTCPDF_IMAGES::_parsejpeg($tempname);
	...
	protected function ImagePngAlpha($file, $x, $y, $wpx, $hpx, $w, $h, $type, $link, $align, $resize, $dpi, $palign, $filehash='') {
		...
		if (($parsed === false) AND extension_loaded('imagick')) {
			try {
				...
				$img->readImage($file);
				// clone image object
				$imga = ColissimoTCPDF_STATIC::objclone($img);
		...
		if ($parsed === false) {
			if (empty($parse_error)) {
				$this->Error('ColissimoTCPDF requires the Imagick or GD extension to handle PNG images with alpha channel.');
			} else {
	...
	public function Ln($h=null, $cell=false) {
		...
		if (ColissimoTCPDF_STATIC::empty_string($h)) {
			$h = $this->lasth;
		}
		$this->y += $h;
		$this->newline = true;
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		if ($this->sign) {
			...
			// remove filler space
			$byterange_string_len = strlen(ColissimoTCPDF_STATIC::$byterange_string);
			// define the ByteRange
			$byte_range = array();
			$byte_range[0] = 0;
			$byte_range[1] = strpos($pdfdoc, ColissimoTCPDF_STATIC::$byterange_string) + $byterange_string_len + 10;
			...
			$pdfdoc = str_replace(ColissimoTCPDF_STATIC::$byterange_string, $byterange, $pdfdoc);
			// write the document to a temporary folder
			$tempdoc = ColissimoTCPDF_STATIC::getObjFilename('doc', $this->file_id);
			$f = ColissimoTCPDF_STATIC::fopenLocal($tempdoc, 'wb');
			if (!$f) {
			...
			$tempsign = ColissimoTCPDF_STATIC::getObjFilename('sig', $this->file_id);
		...
		switch($dest) {
			case 'I': {
				...
				if (php_sapi_name() != 'cli') {
					...
					ColissimoTCPDF_STATIC::sendOutputData($this->getBuffer(), $this->bufferlen);
				} else {
					echo $this->getBuffer();
				}
				break;
			...
			case 'FD': {
				...
				$f = ColissimoTCPDF_STATIC::fopenLocal($name, 'wb');
				if (!$f) {
				...
				if ($dest == 'FI') {
					...
					ColissimoTCPDF_STATIC::sendOutputData(file_get_contents($name), filesize($name));
				} elseif ($dest == 'FD') {
					// send headers to browser
					if (ob_get_contents()) {
						$this->Error('Some data has already been output, can\'t send PDF file');
			...
			if (isset($this->imagekeys)) {
				foreach($this->imagekeys as $file) {
					if ((strpos($file,  K_PATH_CACHE.'__tcpdf_'.$this->file_id.'_') === 0)
						&& ColissimoTCPDF_STATIC::file_exists($file)) {
							$this->_unlink($file);
					}
				}
			}
	...
	protected function getInternalPageNumberAliases($a= '') {
		...
		$u = '{'.$a.'}';
		$alias['u'][] = ColissimoTCPDF_STATIC::_escape($u);
		if ($this->isunicode) {
			$alias['u'][] = ColissimoTCPDF_STATIC::_escape(ColissimoTCPDF_FONTS::UTF8ToLatin1($u, $this->isunicode, $this->CurrentFont));
			$alias['u'][] = ColissimoTCPDF_STATIC::_escape(ColissimoTCPDF_FONTS::utf8StrRev($u, false, $this->tmprtl, $this->isunicode, $this->CurrentFont));
			$alias['a'][] = ColissimoTCPDF_STATIC::_escape(ColissimoTCPDF_FONTS::UTF8ToLatin1($a, $this->isunicode, $this->CurrentFont));
			$alias['a'][] = ColissimoTCPDF_STATIC::_escape(ColissimoTCPDF_FONTS::utf8StrRev($a, false, $this->tmprtl, $this->isunicode, $this->CurrentFont));
		}
		$alias['a'][] = ColissimoTCPDF_STATIC::_escape($a);
		return $alias;
	}
	...
	protected function getAllInternalPageNumberAliases() {
		$basic_alias = array(ColissimoTCPDF_STATIC::$alias_tot_pages, ColissimoTCPDF_STATIC::$alias_num_page, ColissimoTCPDF_STATIC::$alias_group_tot_pages, ColissimoTCPDF_STATIC::$alias_group_num_page, ColissimoTCPDF_STATIC::$alias_right_shift);
		$pnalias = array();
			...
			foreach ($alias as $a) {
				...
				if (($pos = strpos($page, $a)) !== false) {
					...
					if ($type == 'u') {
						...
						$shift = ColissimoTCPDF_FONTS::UTF8ToUTF16BE($shift, false, $this->isunicode, $this->CurrentFont);
					} else {
	...
	protected function setPageBoxTypes($boxes) {
		...
		foreach ($boxes as $box) {
			if (in_array($box, ColissimoTCPDF_STATIC::$pageboxes)) {
				$this->page_boxes[] = $box;
			}
		}
	}
	...
	protected function _putpages() {
		...
		$ptpa = ColissimoTCPDF_STATIC::formatPageNumber(($this->starting_page_number + $num_pages - 1));
		$ptpu = ColissimoTCPDF_FONTS::UTF8ToUTF16BE($ptpa, false, $this->isunicode, $this->CurrentFont);
		...
		for ($n = 1; $n <= $num_pages; ++$n) {
			...
			$pnpa = ColissimoTCPDF_STATIC::formatPageNumber(($this->starting_page_number + $n - 1));
			$pnpu = ColissimoTCPDF_FONTS::UTF8ToUTF16BE($pnpa, false, $this->isunicode, $this->CurrentFont);
			...
			if (!empty($this->pagegroups)) {
				if (isset($this->newpagegroup[$n])) {
					$pagegroupnum = 0;
					++$groupnum;
					$ptga = ColissimoTCPDF_STATIC::formatPageNumber($this->pagegroups[$groupnum]);
					$ptgu = ColissimoTCPDF_FONTS::UTF8ToUTF16BE($ptga, false, $this->isunicode, $this->CurrentFont);
					$ptg_num_chars = $this->GetNumChars($ptga);
				}
				++$pagegroupnum;
				$pnga = ColissimoTCPDF_STATIC::formatPageNumber($pagegroupnum);
				$pngu = ColissimoTCPDF_FONTS::UTF8ToUTF16BE($pnga, false, $this->isunicode, $this->CurrentFont);
				...
				list($temppage, $gdiff) = ColissimoTCPDF_STATIC::replacePageNumAliases($temppage, $replace, $gdiff);
			}
			// replace page numbers
			$replace = array();
			...
			list($temppage, $pdiff) = ColissimoTCPDF_STATIC::replacePageNumAliases($temppage, $replace, $pdiff);
			...
			if (isset($this->PageAnnots[$n])) {
				...
				foreach ($this->PageAnnots[$n] as $key => $pl) {
					...
					if (isset($pl['opt']['c']) AND (is_array($pl['opt']['c'])) AND !empty($pl['opt']['c'])) {
						$annots .= ' /C '.ColissimoTCPDF_COLORS::getColorStringFromArray($pl['opt']['c']);
					}
					//$annots .= ' /StructParent ';
					//$annots .= ' /OC ';
						...
						case 'link': {
							if (is_string($pl['txt']) && !empty($pl['txt'])) {
								if ($pl['txt'][0] == '#') {
									...
									$annots .= ' /A <</S /GoTo /D /'.ColissimoTCPDF_STATIC::encodeNameObject(substr($pl['txt'], 1)).'>>';
								} elseif ($pl['txt'][0] == '%') {
									// embedded PDF file
						...
						case 'widget': {
							...
							if (isset($pl['opt']['mk']) AND (is_array($pl['opt']['mk'])) AND !empty($pl['opt']['mk'])) {
								...
								if (isset($pl['opt']['mk']['bc']) AND (is_array($pl['opt']['mk']['bc']))) {
									$annots .= ' /BC '.ColissimoTCPDF_COLORS::getColorStringFromArray($pl['opt']['mk']['bc']);
								}
								if (isset($pl['opt']['mk']['bg']) AND (is_array($pl['opt']['mk']['bg']))) {
									$annots .= ' /BG '.ColissimoTCPDF_COLORS::getColorStringFromArray($pl['opt']['mk']['bg']);
								}
	...
	protected function _putfonts() {
		...
		foreach ($this->FontFiles as $file => $info) {
			...
			$fontfile = ColissimoTCPDF_FONTS::getFontFullPath($file, $info['fontdir']);
			if (!ColissimoTCPDF_STATIC::empty_string($fontfile)) {
				...
				if ((!$compressed) AND (isset($info['length2']))) {
					...
					$font = ColissimoTCPDF_FONTS::_getTrueTypeFontSubset($font, $subsetchars);
		...
		foreach ($this->fontkeys as $k) {
			...
			if ($type == 'core') {
				...
				if (!ColissimoTCPDF_STATIC::empty_string($font['file'])) {
	...
	protected function _puttruetypeunicode($font) {
		...
		$stream = ColissimoTCPDF_FONT_DATA::$uni_identity_h;
		// ToUnicode Object
		$this->_newobj();
		...
		$out .= "\n".ColissimoTCPDF_FONTS::_putfontwidths($font, 0);
		if (isset($font['ctg']) AND (!ColissimoTCPDF_STATIC::empty_string($font['ctg']))) {
		...
		if (isset($font['ctg']) AND (!ColissimoTCPDF_STATIC::empty_string($font['ctg']))) {
			...
			// search and get ctg font file to embed
			$fontfile = ColissimoTCPDF_FONTS::getFontFullPath($ctgfile, $fontdir);
			if (ColissimoTCPDF_STATIC::empty_string($fontfile)) {
	...
	protected function _putcidfont0($font) {
		...
		$out .= ' /FontDescriptor '.($this->n + 1).' 0 R';
		$out .= ' /DW '.$font['dw'];
		$out .= "\n".ColissimoTCPDF_FONTS::_putfontwidths($font, $cidoffset);
		$out .= ' >>';
		$out .= "\n".'endobj';
		$this->_out($out);
		$this->_newobj();
	...
	protected function _putinfo() {
		...
		if (!ColissimoTCPDF_STATIC::empty_string($this->title)) {
			// The document's title.
			$out .= ' /Title '.$this->_textstring($this->title, $oid);
		}
		if (!ColissimoTCPDF_STATIC::empty_string($this->author)) {
			// The name of the person who created the document.
			$out .= ' /Author '.$this->_textstring($this->author, $oid);
		}
		if (!ColissimoTCPDF_STATIC::empty_string($this->subject)) {
			// The subject of the document.
			$out .= ' /Subject '.$this->_textstring($this->subject, $oid);
		}
		if (!ColissimoTCPDF_STATIC::empty_string($this->keywords)) {
			// Keywords associated with the document.
			$out .= ' /Keywords '.$this->_textstring($this->keywords, $oid);
		}
		if (!ColissimoTCPDF_STATIC::empty_string($this->creator)) {
		...
		// default producer
		$out .= ' /Producer '.$this->_textstring(ColissimoTCPDF_STATIC::getTCPDFProducer(), $oid);
	...
	protected function _putXMP() {
		...
		$this->encrypted = false;
		// set XMP data
		$xmp = '<?xpacket begin="'.ColissimoTCPDF_FONTS::unichr(0xfeff, $this->isunicode).'" id="W5M0MpCehiHzreSzNTczkc9d"?>'."\n";
		...
		$xmp .= "\t\t\t".'<dc:title>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Alt>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li xml:lang="x-default">'.ColissimoTCPDF_STATIC::_escapeXML($this->title).'</rdf:li>'."\n";
		$xmp .= "\t\t\t\t".'</rdf:Alt>'."\n";
		$xmp .= "\t\t\t".'</dc:title>'."\n";
		$xmp .= "\t\t\t".'<dc:creator>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Seq>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li>'.ColissimoTCPDF_STATIC::_escapeXML($this->author).'</rdf:li>'."\n";
		$xmp .= "\t\t\t\t".'</rdf:Seq>'."\n";
		$xmp .= "\t\t\t".'</dc:creator>'."\n";
		$xmp .= "\t\t\t".'<dc:description>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Alt>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li xml:lang="x-default">'.ColissimoTCPDF_STATIC::_escapeXML($this->subject).'</rdf:li>'."\n";
		$xmp .= "\t\t\t\t".'</rdf:Alt>'."\n";
		$xmp .= "\t\t\t".'</dc:description>'."\n";
		$xmp .= "\t\t\t".'<dc:subject>'."\n";
		$xmp .= "\t\t\t\t".'<rdf:Bag>'."\n";
		$xmp .= "\t\t\t\t\t".'<rdf:li>'.ColissimoTCPDF_STATIC::_escapeXML($this->keywords).'</rdf:li>'."\n";
		$xmp .= "\t\t\t\t".'</rdf:Bag>'."\n";
		$xmp .= "\t\t\t".'</dc:subject>'."\n";
		$xmp .= "\t\t".'</rdf:Description>'."\n";
		// convert doc creation date format
		$dcdate = ColissimoTCPDF_STATIC::getFormattedDate($this->doc_creation_timestamp);
		...
		$doccreationdate = ColissimoTCPDF_STATIC::_escapeXML($doccreationdate);
		// convert doc modification date format
		$dmdate = ColissimoTCPDF_STATIC::getFormattedDate($this->doc_modification_timestamp);
		...
		$docmoddate = ColissimoTCPDF_STATIC::_escapeXML($docmoddate);
		...
		$xmp .= "\t\t\t".'<pdf:Keywords>'.ColissimoTCPDF_STATIC::_escapeXML($this->keywords).'</pdf:Keywords>'."\n";
		$xmp .= "\t\t\t".'<pdf:Producer>'.ColissimoTCPDF_STATIC::_escapeXML(ColissimoTCPDF_STATIC::getTCPDFProducer()).'</pdf:Producer>'."\n";
		$xmp .= "\t\t".'</rdf:Description>'."\n";
	...
	protected function _putcatalog() {
		...
		if (isset($this->LayoutMode) AND (!ColissimoTCPDF_STATIC::empty_string($this->LayoutMode))) {
			$out .= ' /PageLayout /'.$this->LayoutMode;
		}
		if (isset($this->PageMode) AND (!ColissimoTCPDF_STATIC::empty_string($this->PageMode))) {
		...
		if (!empty($this->pdflayers)) {
			...
			$out .= ' /Creator '.$this->_textstring('ColissimoTCPDF', $oid);
			$out .= ' /BaseState /ON';
			$out .= ' /OFF ['.$lyrobjs_off.']';
	...
	protected function _putviewerpreferences() {
		...
		if (isset($vp['Duplex']) AND (!ColissimoTCPDF_STATIC::empty_string($vp['Duplex']))) {
	...
	protected function _beginpage($orientation='', $format='') {
		...
		$this->state = 2;
		if (ColissimoTCPDF_STATIC::empty_string($orientation)) {
			if (isset($this->CurOrientation)) {
		...
		if (ColissimoTCPDF_STATIC::empty_string($format)) {
	...
	protected function _getobj($objid=null) {
		if (ColissimoTCPDF_STATIC::empty_string($objid)) {
			++$this->n;
			$objid = $this->n;
		}
	...
	protected function _datastring($s, $n=0) {
		...
		return '('. ColissimoTCPDF_STATIC::_escape($s).')';
	}
	...
	public function setDocCreationTimestamp($time) {
		if (is_string($time)) {
			$time = ColissimoTCPDF_STATIC::getTimestamp($time);
		}
	...
	protected function _datestring($n=0, $timestamp=0) {
		...
		return $this->_datastring('D:'.ColissimoTCPDF_STATIC::getFormattedDate($timestamp), $n);
	}
	...
	protected function _textstring($s, $n=0) {
		if ($this->isunicode) {
			...
			$s = ColissimoTCPDF_FONTS::UTF8ToUTF16BE($s, true, $this->isunicode, $this->CurrentFont);
		}
	...
	protected function _objectkey($n) {
		...
		$objkey = substr(ColissimoTCPDF_STATIC::_md5_16($objkey), 0, (($this->encryptdata['Length'] / 8) + 5));
	...
	protected function _encrypt_data($n, $s) {
		...
		switch ($this->encryptdata['mode']) {
			case 0:   // RC4-40
			case 1: { // RC4-128
				$s = ColissimoTCPDF_STATIC::_RC4($this->_objectkey($n), $s, $this->last_enc_key, $this->last_enc_key_c);
				break;
			}
			case 2: { // AES-128
				$s = ColissimoTCPDF_STATIC::_AES($this->_objectkey($n), $s);
				break;
			}
			case 3: { // AES-256
				$s = ColissimoTCPDF_STATIC::_AES($this->encryptdata['key'], $s);
				break;
			}
		}
		return $s;
	...
	protected function _putencryption() {
		...
		if ($this->encryptdata['pubkey']) {
			...
			if ($this->encryptdata['V'] == 5) { // AES-256
				$out .= ' 5';
				$out .= ' /OE ('.ColissimoTCPDF_STATIC::_escape($this->encryptdata['OE']).')';
				$out .= ' /UE ('.ColissimoTCPDF_STATIC::_escape($this->encryptdata['UE']).')';
				$out .= ' /Perms ('.ColissimoTCPDF_STATIC::_escape($this->encryptdata['perms']).')';
			...
			$out .= ' /O ('.ColissimoTCPDF_STATIC::_escape($this->encryptdata['O']).')';
			$out .= ' /U ('.ColissimoTCPDF_STATIC::_escape($this->encryptdata['U']).')';
	...
	protected function _Uvalue() {
		if ($this->encryptdata['mode'] == 0) { // RC4-40
			return ColissimoTCPDF_STATIC::_RC4($this->encryptdata['key'], ColissimoTCPDF_STATIC::$enc_padding, $this->last_enc_key, $this->last_enc_key_c);
		} elseif ($this->encryptdata['mode'] < 3) { // RC4-128, AES-128
			$tmp = ColissimoTCPDF_STATIC::_md5_16(ColissimoTCPDF_STATIC::$enc_padding.$this->encryptdata['fileid']);
			$enc = ColissimoTCPDF_STATIC::_RC4($this->encryptdata['key'], $tmp, $this->last_enc_key, $this->last_enc_key_c);
			$len = strlen($tmp);
			for ($i = 1; $i <= 19; ++$i) {
				$ek = '';
				for ($j = 0; $j < $len; ++$j) {
				...
				$enc = ColissimoTCPDF_STATIC::_RC4($ek, $enc, $this->last_enc_key, $this->last_enc_key_c);
			}
			$enc .= str_repeat("\x00", 16);
			return substr($enc, 0, 32);
		} elseif ($this->encryptdata['mode'] == 3) { // AES-256
			$seed = ColissimoTCPDF_STATIC::_md5_16(ColissimoTCPDF_STATIC::getRandomSeed());
	...
	protected function _UEvalue() {
		...
		return ColissimoTCPDF_STATIC::_AESnopad($hashkey, $this->encryptdata['key']);
	}
	...
	protected function _Ovalue() {
		if ($this->encryptdata['mode'] < 3) { // RC4-40, RC4-128, AES-128
			$tmp = ColissimoTCPDF_STATIC::_md5_16($this->encryptdata['owner_password']);
			if ($this->encryptdata['mode'] > 0) {
				for ($i = 0; $i < 50; ++$i) {
					$tmp = ColissimoTCPDF_STATIC::_md5_16($tmp);
				}
			}
			$owner_key = substr($tmp, 0, ($this->encryptdata['Length'] / 8));
			$enc = ColissimoTCPDF_STATIC::_RC4($owner_key, $this->encryptdata['user_password'], $this->last_enc_key, $this->last_enc_key_c);
			if ($this->encryptdata['mode'] > 0) {
	...
	protected function _fixAES256Password($password) {
		...
		$psw_array = ColissimoTCPDF_FONTS::utf8Bidi(ColissimoTCPDF_FONTS::UTF8StringToArray($password, $this->isunicode, $this->CurrentFont), $password, $this->rtl, $this->isunicode, $this->CurrentFont);
		foreach ($psw_array as $c) {
			$psw .= ColissimoTCPDF_FONTS::unichr($c, $this->isunicode);
		}
	...
	protected function _generateencryptionkey() {
		...
		if (!$this->encryptdata['pubkey']) { // standard mode
			if ($this->encryptdata['mode'] == 3) { // AES-256
				...
				$this->encryptdata['key'] = substr(hash('sha256', ColissimoTCPDF_STATIC::getRandomSeed(), true), 0, $keybytelen);
				// truncate passwords
				...
				$perms = ColissimoTCPDF_STATIC::getEncPermissionsString($this->encryptdata['protection']); // bytes 0-3
				...
				$this->encryptdata['perms'] = ColissimoTCPDF_STATIC::_AESnopad($this->encryptdata['key'], $perms);
			} else { // RC4-40, RC4-128, AES-128
				// Pad passwords
				$this->encryptdata['user_password'] = substr($this->encryptdata['user_password'].ColissimoTCPDF_STATIC::$enc_padding, 0, 32);
				$this->encryptdata['owner_password'] = substr($this->encryptdata['owner_password'].ColissimoTCPDF_STATIC::$enc_padding, 0, 32);
				// Compute O value
				$this->encryptdata['O'] = $this->_Ovalue();
				// get default permissions (reverse byte order)
				$permissions = ColissimoTCPDF_STATIC::getEncPermissionsString($this->encryptdata['protection']);
				// Compute encryption key
				$tmp = ColissimoTCPDF_STATIC::_md5_16($this->encryptdata['user_password'].$this->encryptdata['O'].$permissions.$this->encryptdata['fileid']);
				if ($this->encryptdata['mode'] > 0) {
					for ($i = 0; $i < 50; ++$i) {
						$tmp = ColissimoTCPDF_STATIC::_md5_16(substr($tmp, 0, $keybytelen));
					}
				}
			...
			// random 20-byte seed
			$seed = sha1(ColissimoTCPDF_STATIC::getRandomSeed(), true);
			$recipient_bytes = '';
			foreach ($this->encryptdata['pubkeys'] as $pubkey) {
				// for each public certificate
				if (isset($pubkey['p'])) {
					$pkprotection = ColissimoTCPDF_STATIC::getUserPermissionCode($pubkey['p'], $this->encryptdata['mode']);
				} else {
					$pkprotection = $this->encryptdata['protection'];
				}
				// get default permissions (reverse byte order)
				$pkpermissions = ColissimoTCPDF_STATIC::getEncPermissionsString($pkprotection);
				// envelope data
				$envelope = $seed.$pkpermissions;
				// write the envelope data to a temporary file
				$tempkeyfile = ColissimoTCPDF_STATIC::getObjFilename('key', $this->file_id);
				$f = ColissimoTCPDF_STATIC::fopenLocal($tempkeyfile, 'wb');
				if (!$f) {
				...
				fclose($f);
				$tempencfile = ColissimoTCPDF_STATIC::getObjFilename('enc', $this->file_id);
	...
	public function setProtection($permissions=array('print', 'modify', 'copy', 'annot-forms', 'fill-forms', 'extract', 'assemble', 'print-high'), $user_pass='', $owner_pass=null, $mode=0, $pubkeys=null) {
		...
		$this->encryptdata['protection'] = ColissimoTCPDF_STATIC::getUserPermissionCode($permissions, $mode);
		...
		if ($owner_pass === null) {
			$owner_pass = md5(ColissimoTCPDF_STATIC::getRandomSeed());
		}
		...
		$this->encrypted = true;
		$this->encryptdata['fileid'] = ColissimoTCPDF_STATIC::convertHexStringToString($this->file_id);
	...
	public function Rect($x, $y, $w, $h, $style='', $border_style=array(), $fill_color=array()) {
		...
		if (!empty($style)) {
			$op = ColissimoTCPDF_STATIC::getPathPaintOperator($style);
	...
	public function Ellipse($x0, $y0, $rx, $ry=0, $angle=0, $astart=0, $afinish=360, $style='', $line_style=array(), $fill_color=array(), $nc=2) {
		...
		if (ColissimoTCPDF_STATIC::empty_string($ry) OR ($ry == 0)) {
			$ry = $rx;
		}
		if (!(false === strpos($style, 'F')) AND is_array($fill_color)) {
			$this->setFillColorArray($fill_color);
	...
	public function setDestination($name, $y=-1, $page='', $x=-1) {
		...
		$name = ColissimoTCPDF_STATIC::encodeNameObject($name);
		if (ColissimoTCPDF_STATIC::empty_string($name)) {
			return false;
		}
		if ($y == -1) {
			$y = $this->GetY();
		...
		foreach ($this->outlines as $i => $o) {
			...
			if (isset($o['u']) AND !empty($o['u'])) {
				...
				if (is_string($o['u'])) {
					if ($o['u'][0] == '#') {
						// internal destination
						$out .= ' /Dest /'.ColissimoTCPDF_STATIC::encodeNameObject(substr($o['u'], 1));
					} elseif ($o['u'][0] == '%') {
						// embedded PDF file
	...
	protected function _addfield($type, $name, $x, $y, $w, $h, $prop) {
		...
		foreach($prop as $key => $val) {
			if (strcmp(substr($key, -5), 'Color') == 0) {
				$val = ColissimoTCPDF_COLORS::_JScolor($val);
			} else {
				$val = "'".$val."'";
			}
	...
	public function TextField($name, $w, $h, $prop=array(), $opt=array(), $x=null, $y=null, $js=false) {
		...
		// get annotation data
		$popt = ColissimoTCPDF_STATIC::getAnnotOptFromJSProp($prop, $this->spot_colors, $this->rtl);
	...
	public function RadioButton($name, $w, $prop=array(), $opt=array(), $onvalue='On', $checked=false, $x=null, $y=null, $js=false) {
		...
		if (ColissimoTCPDF_STATIC::empty_string($onvalue)) {
			$onvalue = 'On';
		}
		if ($checked) {
			$defval = $onvalue;
	...
	protected function _putsignature() {
		...
		$out .= ' '.ColissimoTCPDF_STATIC::$byterange_string;
		...
		if (empty($this->signature_data['approval']) OR ($this->signature_data['approval'] != 'A')) {
			...
			if ($this->signature_data['cert_type'] > 0) {
				...
				$out .= ' /Type /TransformParams';
				$out .= ' /V /2.2';
				if (!ColissimoTCPDF_STATIC::empty_string($this->ur['document'])) {
					$out .= ' /Document['.$this->ur['document'].']';
				}
				if (!ColissimoTCPDF_STATIC::empty_string($this->ur['form'])) {
					$out .= ' /Form['.$this->ur['form'].']';
				}
				if (!ColissimoTCPDF_STATIC::empty_string($this->ur['signature'])) {
					$out .= ' /Signature['.$this->ur['signature'].']';
				}
				if (!ColissimoTCPDF_STATIC::empty_string($this->ur['annots'])) {
					$out .= ' /Annots['.$this->ur['annots'].']';
				}
				if (!ColissimoTCPDF_STATIC::empty_string($this->ur['ef'])) {
					$out .= ' /EF['.$this->ur['ef'].']';
				}
				if (!ColissimoTCPDF_STATIC::empty_string($this->ur['formex'])) {
		...
		if (isset($this->signature_data['info']['Name']) AND !ColissimoTCPDF_STATIC::empty_string($this->signature_data['info']['Name'])) {
			$out .= ' /Name '.$this->_textstring($this->signature_data['info']['Name'], $sigobjid);
		}
		if (isset($this->signature_data['info']['Location']) AND !ColissimoTCPDF_STATIC::empty_string($this->signature_data['info']['Location'])) {
			$out .= ' /Location '.$this->_textstring($this->signature_data['info']['Location'], $sigobjid);
		}
		if (isset($this->signature_data['info']['Reason']) AND !ColissimoTCPDF_STATIC::empty_string($this->signature_data['info']['Reason'])) {
			$out .= ' /Reason '.$this->_textstring($this->signature_data['info']['Reason'], $sigobjid);
		}
		if (isset($this->signature_data['info']['ContactInfo']) AND !ColissimoTCPDF_STATIC::empty_string($this->signature_data['info']['ContactInfo'])) {
	...
	public function getAliasRightShift() {
		...
		$ref = '{'.ColissimoTCPDF_STATIC::$alias_right_shift.'}{'.ColissimoTCPDF_STATIC::$alias_tot_pages.'}{'.ColissimoTCPDF_STATIC::$alias_num_page.'}';
		...
		$alias = ColissimoTCPDF_STATIC::$alias_right_shift.$sdiff.'}';
		if ($this->isUnicodeFont()) {
			$alias = '{'.$alias;
		}
		return $alias;
	...
	public function getAliasNbPages() {
		if ($this->isUnicodeFont()) {
			return '{'.ColissimoTCPDF_STATIC::$alias_tot_pages.'}';
		}
		return ColissimoTCPDF_STATIC::$alias_tot_pages;
	}
	...
	public function getAliasNumPage() {
		if ($this->isUnicodeFont()) {
			return '{'.ColissimoTCPDF_STATIC::$alias_num_page.'}';
		}
		return ColissimoTCPDF_STATIC::$alias_num_page;
	}
	...
	public function getPageGroupAlias() {
		if ($this->isUnicodeFont()) {
			return '{'.ColissimoTCPDF_STATIC::$alias_group_tot_pages.'}';
		}
		return ColissimoTCPDF_STATIC::$alias_group_tot_pages;
	}
	...
	public function getPageNumGroupAlias() {
		if ($this->isUnicodeFont()) {
			return '{'.ColissimoTCPDF_STATIC::$alias_group_num_page.'}';
		}
		return ColissimoTCPDF_STATIC::$alias_group_num_page;
	}
	...
	public function getGroupPageNoFormatted() {
		return ColissimoTCPDF_STATIC::formatPageNumber($this->getGroupPageNo());
	}
	...
	public function PageNoFormatted() {
		return ColissimoTCPDF_STATIC::formatPageNumber($this->PageNo());
	}
	...
	public function setOverprint($stroking=true, $nonstroking=null, $mode=0) {
		...
		if (ColissimoTCPDF_STATIC::empty_string($nonstroking)) {
		...
		foreach ($bars as $col) {
			switch ($col) {
				...
				default: { // SPECIFIC SPOT COLOR NAME
					$col_a = array(0,0,0,0,'None');
					$col_b = ColissimoTCPDF_COLORS::getSpotColor($col, $this->spot_colors);
					if ($col_b === false) {
	...
	public function write1DBarcode($code, $type, $x=null, $y=null, $w=null, $h=null, $xres=null, $style=array(), $align='') {
		if (ColissimoTCPDF_STATIC::empty_string(trim($code))) {
			return;
		}
		...
		$barcodeobj = new ColissimoTCPDFBarcode($code, $type);
		...
		if ($style['stretch']) {
			...
			if (ColissimoTCPDF_STATIC::empty_string($xres)) {
		...
		// height
		if (ColissimoTCPDF_STATIC::empty_string($h) OR ($h <= 0)) {
			// set default height
		...
		// print text
		if ($style['text']) {
			if (isset($style['label']) AND !ColissimoTCPDF_STATIC::empty_string($style['label'])) {
				$label = $style['label'];
			} else {
				$label = $code;
			}
	...
	public function write2DBarcode($code, $type, $x=null, $y=null, $w=null, $h=null, $style=array(), $align='', $distort=false) {
		...
		$gvars = $this->getGraphicVars();
		// create new barcode object
		$barcodeobj = new ColissimoTCPDF2DBarcode($code, $type);
		$arrcode = $barcodeobj->getBarcodeArray();
		...
		// set dimensions
		if ((ColissimoTCPDF_STATIC::empty_string($w) OR ($w <= 0)) AND (ColissimoTCPDF_STATIC::empty_string($h) OR ($h <= 0))) {
	...
	public function fixHTMLCode($html, $default_css='', $tagvs=null, $tidy_options=null) {
		return ColissimoTCPDF_STATIC::fixHTMLCode($html, $default_css, $tagvs, $tidy_options, $this->tagvspaces);
	}
	...
	protected function getCSSBorderStyle($cssborder) {
		...
		$border['color'] = ColissimoTCPDF_COLORS::convertHTMLColorToDec($color, $this->spot_colors);
		return $border;
	}
	...
	public function getHTMLFontUnits($val, $refsize=12, $parent_size=12, $defaultunit='pt') {
		$refsize = ColissimoTCPDF_FONTS::getFontRefSize($refsize);
		$parent_size = ColissimoTCPDF_FONTS::getFontRefSize($parent_size, $refsize);
		switch ($val) {
			case 'xx-small': {
				$size = ($refsize - 4);
				break;
					...
					if (empty($type) OR (isset($type[1]) AND (($type[1] == 'all') OR ($type[1] == 'print')))) {
						...
						if (preg_match('/href[\s]*=[\s]*"([^"]*)"/', $link, $type) > 0) {
							...
							if (($cssdata !== FALSE) AND (strlen($cssdata) > 0)) {
								$css = array_merge($css, ColissimoTCPDF_STATIC::extractCSSproperties($cssdata));
							}
						}
					}
				}
			...
			if (preg_match($tagpattern, $element)) {
				...
				if ($element[0] == '/') {
					...
					if (
						...
						if (ColissimoTCPDF_STATIC::empty_string($dom[($dom[($dom[$key]['parent'])]['parent'])]['thead'])) {
					...
					if (($dom[$key]['value'] == 'table') AND (!ColissimoTCPDF_STATIC::empty_string($dom[($dom[$key]['parent'])]['thead']))) {
					...
					if (!empty($css)) {
						...
						list($dom[$key]['csssel'], $dom[$key]['cssdata']) = ColissimoTCPDF_STATIC::getCSSdataArray($dom, $key, $css);
						$dom[$key]['attribute']['style'] = ColissimoTCPDF_STATIC::getTagStyleFromCSSarray($dom[$key]['cssdata']);
					}
					...
					if (isset($dom[$key]['attribute']['style']) AND !empty($dom[$key]['attribute']['style'])) {
						...
						// font color
						if (isset($dom[$key]['style']['color']) AND (!ColissimoTCPDF_STATIC::empty_string($dom[$key]['style']['color']))) {
							$dom[$key]['fgcolor'] = ColissimoTCPDF_COLORS::convertHTMLColorToDec($dom[$key]['style']['color'], $this->spot_colors);
						} elseif ($dom[$key]['value'] == 'a') {
							$dom[$key]['fgcolor'] = $this->htmlLinkColorArray;
						}
						// background color
						if (isset($dom[$key]['style']['background-color']) AND (!ColissimoTCPDF_STATIC::empty_string($dom[$key]['style']['background-color']))) {
							$dom[$key]['bgcolor'] = ColissimoTCPDF_COLORS::convertHTMLColorToDec($dom[$key]['style']['background-color'], $this->spot_colors);
						}
						// text-decoration
						if (isset($dom[$key]['style']['text-decoration'])) {
							...
							foreach ($decors as $dec) {
								$dec = trim($dec);
								if (!ColissimoTCPDF_STATIC::empty_string($dec)) {
									if ($dec[0] == 'u') {
										// underline
										$dom[$key]['fontstyle'] .= 'U';
									} elseif ($dec[0] == 'l') {
						...
						if (isset($dom[$key]['style']['border-color'])) {
							...
							if (isset($brd_colors[3])) {
								$dom[$key]['border']['L']['color'] = ColissimoTCPDF_COLORS::convertHTMLColorToDec($brd_colors[3], $this->spot_colors);
							}
							if (isset($brd_colors[1])) {
								$dom[$key]['border']['R']['color'] = ColissimoTCPDF_COLORS::convertHTMLColorToDec($brd_colors[1], $this->spot_colors);
							}
							if (isset($brd_colors[0])) {
								$dom[$key]['border']['T']['color'] = ColissimoTCPDF_COLORS::convertHTMLColorToDec($brd_colors[0], $this->spot_colors);
							}
							if (isset($brd_colors[2])) {
								$dom[$key]['border']['B']['color'] = ColissimoTCPDF_COLORS::convertHTMLColorToDec($brd_colors[2], $this->spot_colors);
							}
						}
						...
						foreach ($cellside as $bsk => $bsv) {
							...
							if (isset($dom[$key]['style']['border-'.$bsv.'-color'])) {
								$dom[$key]['border'][$bsk]['color'] = ColissimoTCPDF_COLORS::convertHTMLColorToDec($dom[$key]['style']['border-'.$bsv.'-color'], $this->spot_colors);
							}
					...
					if ((($dom[$key]['value'] == 'ul') OR ($dom[$key]['value'] == 'ol') OR ($dom[$key]['value'] == 'dl'))
						AND (!isset($dom[$key]['align']) OR ColissimoTCPDF_STATIC::empty_string($dom[$key]['align']) OR ($dom[$key]['align'] != 'J'))) {
						if ($this->rtl) {
							$dom[$key]['align'] = 'R';
						} else {
							$dom[$key]['align'] = 'L';
					...
					if (isset($dom[$key]['attribute']['color']) AND (!ColissimoTCPDF_STATIC::empty_string($dom[$key]['attribute']['color']))) {
						$dom[$key]['fgcolor'] = ColissimoTCPDF_COLORS::convertHTMLColorToDec($dom[$key]['attribute']['color'], $this->spot_colors);
					} elseif (!isset($dom[$key]['style']['color']) AND ($dom[$key]['value'] == 'a')) {
						$dom[$key]['fgcolor'] = $this->htmlLinkColorArray;
					}
					// set background color attribute
					if (isset($dom[$key]['attribute']['bgcolor']) AND (!ColissimoTCPDF_STATIC::empty_string($dom[$key]['attribute']['bgcolor']))) {
						$dom[$key]['bgcolor'] = ColissimoTCPDF_COLORS::convertHTMLColorToDec($dom[$key]['attribute']['bgcolor'], $this->spot_colors);
					}
					// set stroke color attribute
					if (isset($dom[$key]['attribute']['strokecolor']) AND (!ColissimoTCPDF_STATIC::empty_string($dom[$key]['attribute']['strokecolor']))) {
						$dom[$key]['strokecolor'] = ColissimoTCPDF_COLORS::convertHTMLColorToDec($dom[$key]['attribute']['strokecolor'], $this->spot_colors);
					}
					// check for width attribute
					...
					// check for text alignment
					if (isset($dom[$key]['attribute']['align']) AND (!ColissimoTCPDF_STATIC::empty_string($dom[$key]['attribute']['align'])) AND ($dom[$key]['value'] !== 'img')) {
	...
	/**
	 * Serialize data to be used with ColissimoTCPDF tag in HTML code.
	 * @param string $method ColissimoTCPDF method name
	...
	/**
	 * Unserialize data to be used with ColissimoTCPDF tag in HTML code.
	...
	/**
	 * Check if a ColissimoTCPDF tag is allowed
	 * @param string $method ColissimoTCPDF method name
	 * @return boolean
	 * @protected
	 */
	...
	public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
		...
		while ($key < $maxel) {
			...
			if (($dom[$key]['value'] == 'tr') AND isset($dom[$key]['thead']) AND $dom[$key]['thead']) {
				if (isset($dom[$key]['parent']) AND isset($dom[$dom[$key]['parent']]['thead']) AND !ColissimoTCPDF_STATIC::empty_string($dom[$dom[$key]['parent']]['thead'])) {
					$this->inthead = true;
			...
			if ($dom[$key]['tag'] OR ($key == 0)) {
				...
				if (ColissimoTCPDF_STATIC::empty_string($lalign)) {
					$lalign = $align;
				}
			}
			// align lines
				...
				if (isset($startlinex)) {
					...
					if ((((($plalign == 'C') OR ($plalign == 'J') OR (($plalign == 'R') AND (!$this->rtl)) OR (($plalign == 'L') AND ($this->rtl)))))) {
						...
						if ($this->isRTLTextDir()) { // RTL
							...
							$pos1 = ColissimoTCPDF_STATIC::revstrpos($pmid, '[(');
							if ($pos1 > 0) {
								$pos1 = intval($pos1);
								if ($this->isUnicodeFont()) {
									$pos2 = intval(ColissimoTCPDF_STATIC::revstrpos($pmid, '[('.chr(0).chr(32)));
									$spacelen = 2;
								} else {
									$pos2 = intval(ColissimoTCPDF_STATIC::revstrpos($pmid, '[('.chr(32)));
									$spacelen = 1;
								}
								if ($pos1 == $pos2) {
							...
							if ($pos1 > 0) {
								$pos1 = intval($pos1);
								if ($this->isUnicodeFont()) {
									$pos2 = intval(ColissimoTCPDF_STATIC::revstrpos($pmid, chr(0).chr(32).')]')) + 2;
									$spacelen = 2;
								} else {
									$pos2 = intval(ColissimoTCPDF_STATIC::revstrpos($pmid, chr(32).')]')) + 1;
									$spacelen = 1;
								}
								if ($pos1 == $pos2) {
							...
							if (preg_match_all('/\[\(([^\)]*)\)\]/x', $pmidtemp, $lnstring, PREG_PATTERN_ORDER)) {
								...
								while (preg_match('/([0-9\.\+\-]*)[\s](Td|cm|m|l|c|re)[\s]/x', $pmid, $strpiece, PREG_OFFSET_CAPTURE, $offset) == 1) {
									...
									switch ($strpiece[2][0]) {
										...
										case 're': {
											// justify block
											if (!ColissimoTCPDF_STATIC::empty_string($this->lispacer)) {
												$this->lispacer = '';
												break;
											}
			...
			if ($dom[$key]['tag']) {
				...
				// print list-item
				if (!ColissimoTCPDF_STATIC::empty_string($this->lispacer) AND ($this->lispacer != '^')) {
				...
				// text
				$this->htmlvspace = 0;
				$isRTLString = preg_match(ColissimoTCPDF_FONT_DATA::$uni_RE_PATTERN_RTL, $dom[$key]['value']) || preg_match(ColissimoTCPDF_FONT_DATA::$uni_RE_PATTERN_ARABIC, $dom[$key]['value']);
				...
				if (!isset($dom[$key]['trimmed_space']) OR !$dom[$key]['trimmed_space']) {
					...
					if (!empty($this->HREF) AND (isset($this->HREF['url']))) {
						...
						if (($strlinelen < $cwa) AND (isset($dom[($key + 1)])) AND ($dom[($key + 1)]['tag']) AND (!$dom[($key + 1)]['block'])) {
							...
							while ($write_block AND isset($dom[$nkey])) {
								if ($dom[$nkey]['tag']) {
									...
									$nextstr = ColissimoTCPDF_STATIC::pregSplit('/'.$this->re_space['p'].'+/', $this->re_space['m'], $dom[$nkey]['value']);
						...
						if (($wadj > 0) AND (($strlinelen + $wadj) >= $cwa)) {
							$wadj = 0;
							$nextstr = ColissimoTCPDF_STATIC::pregSplit('/'.$this->re_space['p'].'/', $this->re_space['m'], $dom[$key]['value']);
		...
		switch($tag['value']) {
			case 'table': {
				...
				if (!isset($dom[$key]['attribute']['nested']) OR ($dom[$key]['attribute']['nested'] != 'true')) {
					$this->htmlvspace = 0;
					// set table header
					if (!ColissimoTCPDF_STATIC::empty_string($dom[$key]['thead'])) {
						// set table header
			...
			case 'img': {
				...
				if ($imgsrc[0] === '@') {
					...
					$type = ColissimoTCPDF_IMAGES::getImageFileType($imgsrc);
				...
				$xpos = $this->x;
				$imglink = '';
				if (isset($this->HREF['url']) AND !ColissimoTCPDF_STATIC::empty_string($this->HREF['url'])) {
			...
			case 'li': {
				...
				if ($this->listordered[$this->listnum]) {
					// ordered item
					if (isset($parent['attribute']['type']) AND !ColissimoTCPDF_STATIC::empty_string($parent['attribute']['type'])) {
						$this->lispacer = $parent['attribute']['type'];
					} elseif (isset($parent['listtype']) AND !ColissimoTCPDF_STATIC::empty_string($parent['listtype'])) {
						$this->lispacer = $parent['listtype'];
					} elseif (isset($this->lisymbol) AND !ColissimoTCPDF_STATIC::empty_string($this->lisymbol)) {
			...
			case 'input': {
				if (isset($tag['attribute']['name']) AND !ColissimoTCPDF_STATIC::empty_string($tag['attribute']['name'])) {
				...
				$prop = array();
				$opt = array();
				if (isset($tag['attribute']['readonly']) AND !ColissimoTCPDF_STATIC::empty_string($tag['attribute']['readonly'])) {
					$prop['readonly'] = true;
				}
				if (isset($tag['attribute']['value']) AND !ColissimoTCPDF_STATIC::empty_string($tag['attribute']['value'])) {
					$value = $tag['attribute']['value'];
				}
				if (isset($tag['attribute']['maxlength']) AND !ColissimoTCPDF_STATIC::empty_string($tag['attribute']['maxlength'])) {
					$opt['maxlen'] = intval($tag['attribute']['maxlength']);
				}
				$h = $this->getCellHeight($this->FontSize);
				if (isset($tag['attribute']['size']) AND !ColissimoTCPDF_STATIC::empty_string($tag['attribute']['size'])) {
				...
				switch ($tag['attribute']['type']) {
					...
					case 'image': {
						...
						if (isset($tag['attribute']['src']) AND !ColissimoTCPDF_STATIC::empty_string($tag['attribute']['src'])) {
			...
			case 'textarea': {
				...
				if (isset($tag['attribute']['cols']) AND !ColissimoTCPDF_STATIC::empty_string($tag['attribute']['cols'])) {
					$w = intval($tag['attribute']['cols']) * $this->GetStringWidth(chr(32)) * 2;
				} else {
					$w = 40;
				}
				if (isset($tag['attribute']['rows']) AND !ColissimoTCPDF_STATIC::empty_string($tag['attribute']['rows'])) {
			...
			case 'select': {
				...
				$w = 0;
				if (isset($tag['attribute']['opt']) AND !ColissimoTCPDF_STATIC::empty_string($tag['attribute']['opt'])) {
			...
			case 'tcpdf': {
				if (defined('K_TCPDF_CALLS_IN_HTML') AND (K_TCPDF_CALLS_IN_HTML === true)) {
					// Special tag used to call ColissimoTCPDF methods
					// This tag is disabled by default by the K_TCPDF_CALLS_IN_HTML constant on ColissimoTCPDF configuration file.
	...
	protected function drawHTMLTagBorder($tag, $xmax) {
		...
		if (isset($tag['border']) AND !empty($tag['border'])) {
			...
			if (!ColissimoTCPDF_STATIC::empty_string($this->thead) AND (!$this->inthead)) {
				...
				$border = ColissimoTCPDF_STATIC::getBorderMode($border, $position='middle', $this->opencell);
			}
		}
	...
	protected function putHtmlListBullet($listdepth, $listtype='', $size=10) {
		...
		switch ($listtype) {
			...
			case 'i':
			case 'lower-roman': {
				$textitem = strtolower(ColissimoTCPDF_STATIC::intToRoman($this->listcount[$this->listnum]));
				break;
			}
			case 'I':
			case 'upper-roman': {
				$textitem = ColissimoTCPDF_STATIC::intToRoman($this->listcount[$this->listnum]);
				break;
			}
			case 'a':
			case 'lower-alpha':
			...
			case 'lower-greek': {
				$textitem = ColissimoTCPDF_FONTS::unichr((945 + $this->listcount[$this->listnum] - 1), $this->isunicode);
				break;
			}
			/*
		...
		if (!ColissimoTCPDF_STATIC::empty_string($textitem)) {
	...
	public function addTOC($page=null, $numbersfont='', $filler='.', $toc_name='TOC', $style='', $color=array(0,0,0)) {
		...
		$page_fill_end = false;
		$current_column = $this->current_column;
		if (ColissimoTCPDF_STATIC::empty_string($numbersfont)) {
			$numbersfont = $this->default_monospaced_font;
		}
		if (ColissimoTCPDF_STATIC::empty_string($filler)) {
			$filler = ' ';
		}
		if (ColissimoTCPDF_STATIC::empty_string($page)) {
			$gap = ' ';
		} else {
			$gap = '';
			if ($page < 1) {
		...
		if (!ColissimoTCPDF_STATIC::empty_string($page)) {
			for ($p = $page_first; $p <= $page_last; ++$p) {
				...
				for ($n = 1; $n <= $maxpage; ++$n) {
					...
					$na = ColissimoTCPDF_STATIC::formatTOCPageNumber(($this->starting_page_number + $np - 1));
					$nu = ColissimoTCPDF_FONTS::UTF8ToUTF16BE($na, false, $this->isunicode, $this->CurrentFont);
					// replace aliases with numbers
					foreach ($pnalias['u'] as $u) {
						$sfill = str_repeat($filler, max(0, (strlen($u) - strlen($nu.' '))));
						if ($this->rtl) {
							$nr = $nu.ColissimoTCPDF_FONTS::UTF8ToUTF16BE(' '.$sfill, false, $this->isunicode, $this->CurrentFont);
						} else {
							$nr = ColissimoTCPDF_FONTS::UTF8ToUTF16BE($sfill.' ', false, $this->isunicode, $this->CurrentFont).$nu;
						}
	...
	/**
	 * Stores a copy of the current ColissimoTCPDF object used for undo operation.
	 * @public
	 * @since 4.5.029 (2009-03-19)
	 */
	public function startTransaction() {
		...
		$this->objcopy = ColissimoTCPDF_STATIC::objclone($this);
	}

	/**
	 * Delete the copy of the current ColissimoTCPDF object used for undo operation.
	 * @public
	 * @since 4.5.029 (2009-03-19)
	 */
	...
	/**
	 * This method allows to undo the latest transaction by returning the latest saved ColissimoTCPDF object with startTransaction().
	...
	public function selectColumn($col=null) {
		if (ColissimoTCPDF_STATIC::empty_string($col)) {
	...
	*/
	...
	protected function hyphenateWord($word, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
		...
		$word_string = ColissimoTCPDF_FONTS::UTF8ArrSubString($word, '', '', $this->isunicode);
		...
		if (isset($dictionary[$word_string])) {
			return ColissimoTCPDF_FONTS::UTF8StringToArray($dictionary[$word_string], $this->isunicode, $this->CurrentFont);
		}
		...
		for ($pos = 0; $pos < $maxpos; ++$pos) {
			...
			for ($i = 1; $i <= $imax; ++$i) {
				$subword = strtolower(ColissimoTCPDF_FONTS::UTF8ArrSubString($tmpword, $pos, ($pos + $i), $this->isunicode));
				if (isset($patterns[$subword])) {
					$pattern = ColissimoTCPDF_FONTS::UTF8StringToArray($patterns[$subword], $this->isunicode, $this->CurrentFont);
	...
	public function hyphenateText($text, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
		...
		if (!is_array($patterns)) {
			$patterns = ColissimoTCPDF_STATIC::getHyphenPatternsFromTEX($patterns);
		}
		// get array of characters
		$unichars = ColissimoTCPDF_FONTS::UTF8StringToArray($text, $this->isunicode, $this->CurrentFont);
		// for each char
		foreach ($unichars as $char) {
			if ((!$intag) AND (!$skip) AND ColissimoTCPDF_FONT_DATA::$uni_type[$char] == 'L') {
				// letter character
				$word[] = $char;
			} else {
				// other type of character
				if (!ColissimoTCPDF_STATIC::empty_string($word)) {
					// hypenate the word
		...
		return ColissimoTCPDF_FONTS::UTF8ArrSubString($txtarr, '', '', $this->isunicode);
	}
	...
	public function printTemplate($id, $x=null, $y=null, $w=0, $h=0, $align='', $palign='', $fitonpage=false) {
		...
		if (!empty($this->xobjects[$id]['annotations'])) {
			foreach ($this->xobjects[$id]['annotations'] as $annot) {
				...
				$coordlt = ColissimoTCPDF_STATIC::getTransformationMatrixProduct($tm, array(1, 0, 0, 1, ($annot['x'] * $this->k), (-$annot['y'] * $this->k)));
				$ax = ($coordlt[4] / $this->k);
				$ay = ($this->h - $h - ($coordlt[5] / $this->k));
				$coordrb = ColissimoTCPDF_STATIC::getTransformationMatrixProduct($tm, array(1, 0, 0, 1, (($annot['x'] + $annot['w']) * $this->k), ((-$annot['y'] - $annot['h']) * $this->k)));
	...
	protected function setSVGStyles($svgstyle, $prevsvgstyle, $x=0, $y=0, $w=1, $h=1, $clip_function='', $clip_params=array()) {
		...
		// color
		$fill_color = ColissimoTCPDF_COLORS::convertHTMLColorToDec($svgstyle['color'], $this->spot_colors);
		$this->setFillColorArray($fill_color);
		// text color
		$text_color = ColissimoTCPDF_COLORS::convertHTMLColorToDec($svgstyle['text-color'], $this->spot_colors);
		...
		if (preg_match('/url\([\s]*\#([^\)]*)\)/si', $svgstyle['fill'], $regs)) {
			...
			$fill_color = ColissimoTCPDF_COLORS::convertHTMLColorToDec($svgstyle['fill'], $this->spot_colors);
		...
		if ($svgstyle['stroke'] != 'none') {
			...
			$stroke_style = array(
				'color' => ColissimoTCPDF_COLORS::convertHTMLColorToDec($svgstyle['stroke'], $this->spot_colors),
	...
	protected function SVGPath($d, $style='') {
		...
		foreach ($paths as $key => $val) {
			...
			$params = array();
			if (isset($val[2])) {
				// get curve parameters, see https://github.com/tecnickcom/ColissimoTCPDF/issues/767
					...
					foreach ($params as $ck => $cp) {
						...
						if ((($ck + 1) % 7) == 0) {
							...
							if ((abs($x0 - $x) < $minlen) AND (abs($y0 - $y) < $minlen)) {
								...
								// get angles
								$angs = ColissimoTCPDF_STATIC::getVectorsAngle(1, 0, (($xa - $cax) / $rx), (($cay - $ya) / $ry));
								$dang = ColissimoTCPDF_STATIC::getVectorsAngle((($xa - $cax) / $rx), (($ya - $cay) / $ry), ((-$xa - $cax) / $rx), ((-$ya - $cay) / $ry));
								if (($fs == 0) AND ($dang > 0)) {
									$dang -= (2 * M_PI);
	...
	protected function startSVGElementHandler($parser, $name, $attribs, $ctm=array()) {
		...
		if (isset($attribs['style']) AND !ColissimoTCPDF_STATIC::empty_string($attribs['style']) AND ($attribs['style'][0] != ';')) {
			// fix style for regular expression
			$attribs['style'] = ';'.$attribs['style'];
		}
		foreach ($prev_svgstyle as $key => $val) {
			if (in_array($key, ColissimoTCPDF_IMAGES::$svginheritprop)) {
				// inherit previous value
				$svgstyle[$key] = $val;
			}
			if (isset($attribs[$key]) AND !ColissimoTCPDF_STATIC::empty_string($attribs[$key])) {
			...
			} elseif (isset($attribs['style']) AND !ColissimoTCPDF_STATIC::empty_string($attribs['style'])) {
				// CSS style syntax
				$attrval = array();
		...
		if (isset($attribs['transform']) AND !empty($attribs['transform'])) {
			$tm = ColissimoTCPDF_STATIC::getTransformationMatrixProduct($tm, ColissimoTCPDF_STATIC::getSVGTransformMatrix($attribs['transform']));
		}
		...
		switch($name) {
			...
			case 'svg': {
				...
				$tm = ColissimoTCPDF_STATIC::getTransformationMatrixProduct($tm, array( 1, 0, 0, 1, $svgX, $svgY));
				...
				if (isset($attribs['viewBox'])) {
					...
					if (sizeof($tmp) == 4) {
						...
						$tm = ColissimoTCPDF_STATIC::getTransformationMatrixProduct($tm, $newtm);
						$this->SVGTransform($tm);
					}
				}
			...
			case 'g': {
				...
				$tm = ColissimoTCPDF_STATIC::getTransformationMatrixProduct($tm, array($w, 0, 0, $h, $x, $y));
				$this->SVGTransform($tm);
				$this->setSVGStyles($svgstyle, $prev_svgstyle);
				break;
			}
			case 'linearGradient': {
				...
				if (isset($attribs['gradientTransform'])) {
					$this->svggradients[$this->svggradientid]['gradientTransform'] = ColissimoTCPDF_STATIC::getSVGTransformMatrix($attribs['gradientTransform']);
				}
			...
			case 'stop': {
				...
				$stop_color = isset($svgstyle['stop-color'])?ColissimoTCPDF_COLORS::convertHTMLColorToDec($svgstyle['stop-color'], $this->spot_colors):'black';
			...
			case 'image': {
				...
				if (!$clipping) {
					...
					if (preg_match('/^data:image\/[^;]+;base64,/', $img, $m) > 0) {
						...
						if (!ColissimoTCPDF_STATIC::empty_string($this->svgdir) AND (($img[0] == '.') OR (basename($img) == $img))) {
					...
					// get image type
					$imgtype = ColissimoTCPDF_IMAGES::getImageFileType($img);
					if (($imgtype == 'eps') OR ($imgtype == 'ai')) {
    ...
    protected function getCachedFileContents($file)
    {
        if (!isset($this->fileContentCache[$file])) {
            $this->fileContentCache[$file] = ColissimoTCPDF_STATIC::fileGetContents($file);
        }
    ...
    protected function fileExists($file)
    {
        ...
        return ColissimoTCPDF_STATIC::file_exists($file);
    }

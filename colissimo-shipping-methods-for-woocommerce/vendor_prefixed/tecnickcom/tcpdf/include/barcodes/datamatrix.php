...
// DESCRIPTION :
//
// Class to create DataMatrix ECC 200 barcode arrays for ColissimoTCPDF class.
...
// custom definitions
if (!defined('COLISSIMO_DATAMATRIXDEFS')) {

	/**
	 * Indicate that definitions for this class are set
	 */
	define('COLISSIMO_DATAMATRIXDEFS', true);
...
class ColissimoDatamatrix {
	 ...
	 * @protected
	 */
	protected $last_enc = COLISSIMO_ENC_ASCII;
	 ...
	 * @protected
	 */
	protected $chset_id = array(COLISSIMO_ENC_C40 => 'C40', COLISSIMO_ENC_TXT => 'TXT', COLISSIMO_ENC_X12 =>'X12');
...
// -----------------------------------------------------------------------------
	 ...
	 * @param string $code Code to represent using ColissimoDatamatrix.
	 * @public
	 */
	public function __construct($code) {
		...
		if ($params[11] < $nd) {
			...
			if ((($params[11] - $nd) > 1) AND ($cw[($nd - 1)] != 254)) {
				if ($this->last_enc == COLISSIMO_ENC_EDF) {
					// switch to ASCII encoding
					$cw[] = 124;
					++$nd;
				} elseif (($this->last_enc != COLISSIMO_ENC_ASCII) AND ($this->last_enc != COLISSIMO_ENC_BASE256)) {
					// switch to ASCII encoding
					$cw[] = 254;
					++$nd;
				}
	...
	protected function isCharMode($chr, $mode) {
		$status = false;
		switch ($mode) {
			case COLISSIMO_ENC_ASCII: { // ASCII character 0 to 127
				$status = (($chr >= 0) AND ($chr <= 127));
				break;
			}
			case COLISSIMO_ENC_C40: { // Upper-case alphanumeric
				$status = (($chr == 32) OR (($chr >= 48) AND ($chr <= 57)) OR (($chr >= 65) AND ($chr <= 90)));
				break;
			}
			case COLISSIMO_ENC_TXT: { // Lower-case alphanumeric
				$status = (($chr == 32) OR (($chr >= 48) AND ($chr <= 57)) OR (($chr >= 97) AND ($chr <= 122)));
				break;
			}
			case COLISSIMO_ENC_X12: { // ANSI X12
				$status = (($chr == 13) OR ($chr == 42) OR ($chr == 62));
				break;
			}
			case COLISSIMO_ENC_EDF: { // ASCII character 32 to 94
				$status = (($chr >= 32) AND ($chr <= 94));
				break;
			}
			case COLISSIMO_ENC_BASE256: { // Function character (FNC1, Structured Append, Reader Program, or Code Page)
				$status = (($chr == 232) OR ($chr == 233) OR ($chr == 234) OR ($chr == 241));
				break;
			}
			case COLISSIMO_COLISSIMO_ENC_ASCII_EXT: { // ASCII character 128 to 255
				$status = (($chr >= 128) AND ($chr <= 255));
				break;
			}
			case COLISSIMO_COLISSIMO_ENC_ASCII_NUM: { // ASCII digits
	...
	protected function lookAheadTest($data, $pos, $mode) {
		...
		while (true) {
			...
			if (($pos + $charscount) == $data_length) {
				if ($numch[COLISSIMO_ENC_ASCII] <= ceil(min($numch[COLISSIMO_ENC_C40], $numch[COLISSIMO_ENC_TXT], $numch[COLISSIMO_ENC_X12], $numch[COLISSIMO_ENC_EDF], $numch[COLISSIMO_ENC_BASE256]))) {
					return COLISSIMO_ENC_ASCII;
				}
				if ($numch[COLISSIMO_ENC_BASE256] < ceil(min($numch[COLISSIMO_ENC_ASCII], $numch[COLISSIMO_ENC_C40], $numch[COLISSIMO_ENC_TXT], $numch[COLISSIMO_ENC_X12], $numch[COLISSIMO_ENC_EDF]))) {
					return COLISSIMO_ENC_BASE256;
				}
				if ($numch[COLISSIMO_ENC_EDF] < ceil(min($numch[COLISSIMO_ENC_ASCII], $numch[COLISSIMO_ENC_C40], $numch[COLISSIMO_ENC_TXT], $numch[COLISSIMO_ENC_X12], $numch[COLISSIMO_ENC_BASE256]))) {
					return COLISSIMO_ENC_EDF;
				}
				if ($numch[COLISSIMO_ENC_TXT] < ceil(min($numch[COLISSIMO_ENC_ASCII], $numch[COLISSIMO_ENC_C40], $numch[COLISSIMO_ENC_X12], $numch[COLISSIMO_ENC_EDF], $numch[COLISSIMO_ENC_BASE256]))) {
					return COLISSIMO_ENC_TXT;
				}
				if ($numch[COLISSIMO_ENC_X12] < ceil(min($numch[COLISSIMO_ENC_ASCII], $numch[COLISSIMO_ENC_C40], $numch[COLISSIMO_ENC_TXT], $numch[COLISSIMO_ENC_EDF], $numch[COLISSIMO_ENC_BASE256]))) {
					return COLISSIMO_ENC_X12;
				}
				return COLISSIMO_ENC_C40;
			}
			// get char
			$chr = ord($data[$pos + $charscount]);
			$charscount++;
			// STEP L
			if ($this->isCharMode($chr, COLISSIMO_COLISSIMO_ENC_ASCII_NUM)) {
				$numch[COLISSIMO_ENC_ASCII] += (1 / 2);
			} elseif ($this->isCharMode($chr, COLISSIMO_COLISSIMO_ENC_ASCII_EXT)) {
				$numch[COLISSIMO_ENC_ASCII] = ceil($numch[COLISSIMO_ENC_ASCII]);
				$numch[COLISSIMO_ENC_ASCII] += 2;
			} else {
				$numch[COLISSIMO_ENC_ASCII] = ceil($numch[COLISSIMO_ENC_ASCII]);
				$numch[COLISSIMO_ENC_ASCII] += 1;
			}
			// STEP M
			if ($this->isCharMode($chr, COLISSIMO_ENC_C40)) {
				$numch[COLISSIMO_ENC_C40] += (2 / 3);
			} elseif ($this->isCharMode($chr, COLISSIMO_COLISSIMO_ENC_ASCII_EXT)) {
				$numch[COLISSIMO_ENC_C40] += (8 / 3);
			} else {
				$numch[COLISSIMO_ENC_C40] += (4 / 3);
			}
			// STEP N
			if ($this->isCharMode($chr, COLISSIMO_ENC_TXT)) {
				$numch[COLISSIMO_ENC_TXT] += (2 / 3);
			} elseif ($this->isCharMode($chr, COLISSIMO_COLISSIMO_ENC_ASCII_EXT)) {
				$numch[COLISSIMO_ENC_TXT] += (8 / 3);
			} else {
				$numch[COLISSIMO_ENC_TXT] += (4 / 3);
			}
			// STEP O
			if ($this->isCharMode($chr, COLISSIMO_ENC_X12) OR $this->isCharMode($chr, COLISSIMO_ENC_C40)) {
				$numch[COLISSIMO_ENC_X12] += (2 / 3);
			} elseif ($this->isCharMode($chr, COLISSIMO_COLISSIMO_ENC_ASCII_EXT)) {
				$numch[COLISSIMO_ENC_X12] += (13 / 3);
			} else {
				$numch[COLISSIMO_ENC_X12] += (10 / 3);
			}
			// STEP P
			if ($this->isCharMode($chr, COLISSIMO_ENC_EDF)) {
				$numch[COLISSIMO_ENC_EDF] += (3 / 4);
			} elseif ($this->isCharMode($chr, COLISSIMO_COLISSIMO_ENC_ASCII_EXT)) {
				$numch[COLISSIMO_ENC_EDF] += (17 / 4);
			} else {
				$numch[COLISSIMO_ENC_EDF] += (13 / 4);
			}
			// STEP Q
			if ($this->isCharMode($chr, COLISSIMO_ENC_BASE256)) {
				$numch[COLISSIMO_ENC_BASE256] += 4;
			} else {
				$numch[COLISSIMO_ENC_BASE256] += 1;
			}
			// STEP R
			if ($charscount >= 4) {
				if (($numch[COLISSIMO_ENC_ASCII] + 1) <= min($numch[COLISSIMO_ENC_C40], $numch[COLISSIMO_ENC_TXT], $numch[COLISSIMO_ENC_X12], $numch[COLISSIMO_ENC_EDF], $numch[COLISSIMO_ENC_BASE256])) {
					return COLISSIMO_ENC_ASCII;
				}
				if ((($numch[COLISSIMO_ENC_BASE256] + 1) <= $numch[COLISSIMO_ENC_ASCII])
					OR (($numch[COLISSIMO_ENC_BASE256] + 1) < min($numch[COLISSIMO_ENC_C40], $numch[COLISSIMO_ENC_TXT], $numch[COLISSIMO_ENC_X12], $numch[COLISSIMO_ENC_EDF]))) {
					return COLISSIMO_ENC_BASE256;
				}
				if (($numch[COLISSIMO_ENC_EDF] + 1) < min($numch[COLISSIMO_ENC_ASCII], $numch[COLISSIMO_ENC_C40], $numch[COLISSIMO_ENC_TXT], $numch[COLISSIMO_ENC_X12], $numch[COLISSIMO_ENC_BASE256])) {
					return COLISSIMO_ENC_EDF;
				}
				if (($numch[COLISSIMO_ENC_TXT] + 1) < min($numch[COLISSIMO_ENC_ASCII], $numch[COLISSIMO_ENC_C40], $numch[COLISSIMO_ENC_X12], $numch[COLISSIMO_ENC_EDF], $numch[COLISSIMO_ENC_BASE256])) {
					return COLISSIMO_ENC_TXT;
				}
				if (($numch[COLISSIMO_ENC_X12] + 1) < min($numch[COLISSIMO_ENC_ASCII], $numch[COLISSIMO_ENC_C40], $numch[COLISSIMO_ENC_TXT], $numch[COLISSIMO_ENC_EDF], $numch[COLISSIMO_ENC_BASE256])) {
					return COLISSIMO_ENC_X12;
				}
				if (($numch[COLISSIMO_ENC_C40] + 1) < min($numch[COLISSIMO_ENC_ASCII], $numch[COLISSIMO_ENC_TXT], $numch[COLISSIMO_ENC_EDF], $numch[COLISSIMO_ENC_BASE256])) {
					if ($numch[COLISSIMO_ENC_C40] < $numch[COLISSIMO_ENC_X12]) {
						return COLISSIMO_ENC_C40;
					}
					if ($numch[COLISSIMO_ENC_C40] == $numch[COLISSIMO_ENC_X12]) {
						$k = ($pos + $charscount + 1);
						while ($k < $data_length) {
							$tmpchr = ord($data[$k]);
							if ($this->isCharMode($tmpchr, COLISSIMO_ENC_X12)) {
								return COLISSIMO_ENC_X12;
							} elseif (!($this->isCharMode($tmpchr, COLISSIMO_ENC_X12) OR $this->isCharMode($tmpchr, COLISSIMO_ENC_C40))) {
								break;
							}
							++$k;
						}
	...
	protected function getHighLevelEncoding($data) {
		...
		$enc = COLISSIMO_ENC_ASCII; // current encoding mode
		...
		while ($pos < $data_length) {
			...
			$this->last_enc = $enc;
			switch ($enc) {
				case COLISSIMO_ENC_ASCII: { // STEP B. While in ASCII encodation
					if (($data_length > 1) AND ($pos < ($data_length - 1)) AND ($this->isCharMode(ord($data[$pos]), COLISSIMO_COLISSIMO_ENC_ASCII_NUM) AND $this->isCharMode(ord($data[$pos + 1]), COLISSIMO_COLISSIMO_ENC_ASCII_NUM))) {
						...
						if ($newenc != $enc) {
							...
							$chr = ord($data[$pos]);
							++$pos;
							if ($this->isCharMode($chr, COLISSIMO_COLISSIMO_ENC_ASCII_EXT)) {
				...
				case COLISSIMO_ENC_X12 : { // ANSI X12
					...
					do {
						...
						if (isset($charset[$chr])) {
							...
							} elseif (($enc == COLISSIMO_ENC_C40) AND isset($this->chset['S3C'][$chr])) {
								$temp_cw[] = 2; // shift 3
								$shiftset = $this->chset['S3C'];
							} elseif (($enc == COLISSIMO_ENC_TXT) AND isset($this->chset['S3T'][$chr])) {
								$temp_cw[] = 2; // shift 3
						...
						if ($p >= 3) {
							...
							if ($newenc != $enc) {
								// switch to new encoding
								$enc = $newenc;
								if ($enc != COLISSIMO_ENC_ASCII) {
									// set unlatch character
									$cw[] = $this->getSwitchEncodingCodeword(COLISSIMO_ENC_ASCII);
									++$cw_num;
								}
								$cw[] = $this->getSwitchEncodingCodeword($enc);
								++$cw_num;
				...
				case COLISSIMO_ENC_EDF: { // F. While in EDIFACT (EDF) encodation
					...
					do {
						...
						if (($field_length == 4) OR ($epos == $data_length) OR !$this->isCharMode($chr, COLISSIMO_ENC_EDF)) {
				...
				case COLISSIMO_ENC_BASE256: { // G. While in Base 256 (B256) encodation
					// initialize temporary array with 0 length
					$temp_cw = array();
					$field_length = 0;

...
// DESCRIPTION :
//
// Class to create ColissimoPDF417 barcode arrays for ColissimoTCPDF class.
// ColissimoPDF417 (ISO/IEC 15438:2006) is a 2-dimensional stacked bar code created by Symbol Technologies in 1991.
...
// TECHNICAL DATA / FEATURES OF ColissimoPDF417:
...
// definitions
if (!defined('COLISSIMO_PDF417DEFS')) {

	/**
	 * Indicate that definitions for this class are set
	 */
	define('COLISSIMO_PDF417DEFS', true);
	 ...
	 */
	define('COLISSIMO_ROWHEIGHT', 4);

	/**
	 * Horizontal quiet zone in modules
	 */
	define('COLISSIMO_QUIETH', 2);

	/**
	 * Vertical quiet zone in modules
	 */
	define('COLISSIMO_QUIETV', 2);
...
class ColissimoPDF417 {
	 ...
	 * Creates a ColissimoPDF417 object
	 * @param string $code code to represent using ColissimoPDF417
	...
	public function __construct($code, $ecl=-1, $aspectratio=2, $macro=array()) {
		...
		$nce = ($numcw + $errsize + 1);
		$cols = round((sqrt(4761 + (68 * $aspectratio * COLISSIMO_ROWHEIGHT * $nce)) - 69) / 34);
		// adjust cols
		if ($cols < 1) {
			$cols = 1;
		} elseif ($cols > 30) {
		...
		$pstart = str_repeat('0', COLISSIMO_QUIETH).$this->start_pattern;
		$pstop = $this->stop_pattern.str_repeat('0', COLISSIMO_QUIETH);
		$barcode_array['num_rows'] = ($rows * COLISSIMO_ROWHEIGHT) + (2 * COLISSIMO_QUIETV);
		$barcode_array['num_cols'] = (($cols + 2) * 17) + 35 + (2 * COLISSIMO_QUIETH);
		$barcode_array['bcode'] = array();
		// build rows for vertical quiet zone
		if (COLISSIMO_QUIETV > 0) {
			$empty_row = array_fill(0, $barcode_array['num_cols'], 0);
			for ($i = 0; $i < COLISSIMO_QUIETV; ++$i) {
		...
		for ($r = 0; $r < $rows; ++$r) {
			...
			for ($h = 0; $h < COLISSIMO_ROWHEIGHT; ++$h) {

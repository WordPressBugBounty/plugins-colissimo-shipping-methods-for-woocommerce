...
// DESCRIPTION :
//
// Class to create QR-code arrays for ColissimoTCPDF class.
...
// definitions
if (!defined('COLISSIMO_QRCODEDEFS')) {

	/**
	 * Indicate that definitions for this class are set
	 */
	define('COLISSIMO_QRCODEDEFS', true);

	// -----------------------------------------------------

	// Encoding modes (characters which can be encoded in ColissimoQRcode)

	/**
	 * Encoding mode
	 */
	define('COLISSIMO_QR_MODE_NL', -1);

	/**
	 * Encoding mode numeric (0-9). 3 characters are encoded to 10bit length. In theory, 7089 characters or less can be stored in a ColissimoQRcode.
	 */
	define('COLISSIMO_QR_MODE_NM', 0);

	/**
	 * Encoding mode alphanumeric (0-9A-Z $%*+-./:) 45characters. 2 characters are encoded to 11bit length. In theory, 4296 characters or less can be stored in a ColissimoQRcode.
	 */
	define('COLISSIMO_QR_MODE_AN', 1);

	/**
	 * Encoding mode 8bit byte data. In theory, 2953 characters or less can be stored in a ColissimoQRcode.
	 */
	define('COLISSIMO_QR_MODE_8B', 2);

	/**
	 * Encoding mode KANJI. A KANJI character (multibyte character) is encoded to 13bit length. In theory, 1817 characters or less can be stored in a ColissimoQRcode.
	 */
	define('COLISSIMO_QR_MODE_KJ', 3);

	/**
	 * Encoding mode STRUCTURED (currently unsupported)
	 */
	define('COLISSIMO_QR_MODE_ST', 4);

	// -----------------------------------------------------

	// Levels of error correction.
	// ColissimoQRcode has a function of an error correcting for miss reading that white is black.
	// Error correcting is defined in 4 level as below.

	/**
	 * Error correction level L : About 7% or less errors can be corrected.
	 */
	define('COLISSIMO_QR_ECLEVEL_L', 0);

	/**
	 * Error correction level M : About 15% or less errors can be corrected.
	 */
	define('COLISSIMO_QR_ECLEVEL_M', 1);

	/**
	 * Error correction level Q : About 25% or less errors can be corrected.
	 */
	define('COLISSIMO_QR_ECLEVEL_Q', 2);

	/**
	 * Error correction level H : About 30% or less errors can be corrected.
	 */
	define('COLISSIMO_QR_ECLEVEL_H', 3);

	// -----------------------------------------------------

	// Version. Size of ColissimoQRcode is defined as version.
	 ...
	 */
	define('COLISSIMO_QRSPEC_VERSION_MAX', 40);

	/**
	 * Maximum matrix size for maximum version (version 40 is 177*177 matrix).
	 */
    define('COLISSIMO_QRSPEC_WIDTH_MAX', 177);
	 ...
	 */
    define('COLISSIMO_QRCAP_WIDTH',    0);

    /**
	 * Matrix index to get number of words from $capacity array.
	 */
    define('COLISSIMO_QRCAP_WORDS',    1);

    /**
	 * Matrix index to get remainder from $capacity array.
	 */
    define('COLISSIMO_QRCAP_REMINDER', 2);

    /**
	 * Matrix index to get error correction level from $capacity array.
	 */
    define('COLISSIMO_QRCAP_EC',       3);
	 ...
	 */
    define('COLISSIMO_STRUCTURE_HEADER_BITS',  20);

    /**
	 * Max number of symbols for structured mode
	 */
    define('COLISSIMO_MAX_STRUCTURED_SYMBOLS', 16);
	 ...
	 */
    define('COLISSIMO_N1',  3);

    /**
	 * Down point base value for case 2 mask pattern (module block of same color)
	 */
	define('COLISSIMO_N2',  3);

    /**
	 * Down point base value for case 3 mask pattern (1:1:3:1:1(dark:bright:dark:bright:dark)pattern in a line or a column)
	 */
	define('COLISSIMO_N3', 40);

    /**
	 * Down point base value for case 4 mask pattern (ration of dark modules in whole)
	 */
	define('COLISSIMO_N4', 10);
	 ...
	 */
	define('COLISSIMO_QR_FIND_BEST_MASK', true);

	/**
	 * if false, checks all masks available, otherwise value tells count of masks need to be checked, mask id are got randomly
	 */
	define('COLISSIMO_QR_FIND_FROM_RANDOM', 2);

	/**
	 * when COLISSIMO_QR_FIND_BEST_MASK === false
	 */
	define('COLISSIMO_QR_DEFAULT_MASK', 2);
...
class ColissimoQRcode {
	...
	/**
	 * Barcode array to be returned which is readable by ColissimoTCPDF.
	 * @protected
	 */
	protected $barcode_array = array();

	/**
	 * QR code version. Size of ColissimoQRcode is defined as version. Version is from 1 to 40. Version 1 is 21*21 matrix. And 4 modules increases whenever 1 version increases. So version 40 is 177*177 matrix.
	 * @protected
	 */
	protected $version = 0;
	 ...
	 * @protected
	 */
	protected $level = COLISSIMO_QR_ECLEVEL_L;

	/**
	 * Encoding mode.
	 * @protected
	 */
	protected $hint = COLISSIMO_QR_MODE_8B;
	 ...
	 * size: [COLISSIMO_QRSPEC_VERSION_MAX - 6]
	 * @protected
	 */
	 ...
	 * Creates a ColissimoQRcode object
	 * @param string $code code to represent using ColissimoQRcode
	...
	public function __construct($code, $eclevel = 'L') {
		...
		if ($this->level === false) {
			$this->level = COLISSIMO_QR_ECLEVEL_L;
		}
		if (($this->hint != COLISSIMO_QR_MODE_8B) AND ($this->hint != COLISSIMO_QR_MODE_KJ)) {
			return false;
		}
		if (($this->version < 0) OR ($this->version > COLISSIMO_QRSPEC_VERSION_MAX)) {
			return false;
		}
		$this->items = array();
	...
	/**
	 * Returns a barcode array which is readable by ColissimoTCPDF
	 * @return array barcode array readable by ColissimoTCPDF;
	 * @public
	 */
	...
	protected function encodeMask($mask) {
		...
		// masking
		$this->runLength = array_fill(0, COLISSIMO_QRSPEC_WIDTH_MAX + 1, 0);
		if ($mask < 0) {
			if (COLISSIMO_QR_FIND_BEST_MASK) {
				$masked = $this->mask($this->width, $this->frame, $this->level);
			} else {
				$masked = $this->makeMask($this->width, $this->frame, (intval(COLISSIMO_QR_DEFAULT_MASK) % 8), $this->level);
			}
		} else {
	...
	/**
	 * calcCOLISSIMO_N1COLISSIMO_N3
	 * @param int $length
	 * @return int demerit
	 */
	 protected function calcCOLISSIMO_N1COLISSIMO_N3($length) {
		$demerit = 0;
		for ($i=0; $i<$length; ++$i) {
			if ($this->runLength[$i] >= 5) {
				$demerit += (COLISSIMO_N1 + ($this->runLength[$i] - 5));
			}
			if ($i & 1) {
	 ...
	 */
	 protected function evaluateSymbol($width, $frame) {
		...
		for ($y=0; $y<$width; ++$y) {
			...
			$demerit += $this->calcCOLISSIMO_N1COLISSIMO_N3($head+1);
		}
		for ($x=0; $x<$width; ++$x) {
			$head = 0;
			$this->runLength[0] = 1;
	 ...
	 */
	 protected function mask($width, $frame, $level) {
		...
		if (COLISSIMO_QR_FIND_FROM_RANDOM !== false) {
			$howManuOut = 8 - (COLISSIMO_QR_FIND_FROM_RANDOM % 9);
		...
		foreach ($checked_masks as $i) {
			...
			$demerit = (int)((int)(abs($blacks - 50) / 5) * COLISSIMO_N4);
	 ...
	 */
	 protected function identifyMode($pos) {
		if ($pos >= strlen($this->dataStr)) {
			return COLISSIMO_QR_MODE_NL;
		}
		$c = $this->dataStr[$pos];
		if ($this->isdigitat($this->dataStr, $pos)) {
			return COLISSIMO_QR_MODE_NM;
		} elseif ($this->isalnumat($this->dataStr, $pos)) {
			return COLISSIMO_QR_MODE_AN;
		} elseif ($this->hint == COLISSIMO_QR_MODE_KJ) {
			if ($pos+1 < strlen($this->dataStr)) {
				$d = $this->dataStr[$pos+1];
				$word = (ord($c) << 8) | ord($d);
				if (($word >= 0x8140 && $word <= 0x9ffc) OR ($word >= 0xe040 && $word <= 0xebbf)) {
					return COLISSIMO_QR_MODE_KJ;
				}
			}
		}
		return COLISSIMO_QR_MODE_8B;
	}
	 ...
	 */
	 protected function eatNum() {
		$ln = $this->lengthIndicator(COLISSIMO_QR_MODE_NM, $this->version);
		$p = 0;
		...
		$this->items = $this->appendNewInputItem($this->items, COLISSIMO_QR_MODE_NM, $run, str_split($this->dataStr));
		return $run;
	}
	 ...
	 */
	 protected function eatAn() {
		$la = $this->lengthIndicator(COLISSIMO_QR_MODE_AN,  $this->version);
		$ln = $this->lengthIndicator(COLISSIMO_QR_MODE_NM, $this->version);
		$p =1 ;
		...
		$this->items = $this->appendNewInputItem($this->items, COLISSIMO_QR_MODE_AN, $run, str_split($this->dataStr));
		return $run;
	}
	 ...
	 */
	 protected function eatKanji() {
		$p = 0;
		while($this->identifyMode($p) == COLISSIMO_QR_MODE_KJ) {
			$p += 2;
		}
		$this->items = $this->appendNewInputItem($this->items, COLISSIMO_QR_MODE_KJ, $p, str_split($this->dataStr));
		$run = $p;
		return $run;
	}
	 ...
	 */
	 protected function eat8() {
		...
		while($p < $dataStrLen) {
			$mode = $this->identifyMode($p);
			if ($mode == COLISSIMO_QR_MODE_KJ) {
				break;
			}
			if ($mode == COLISSIMO_QR_MODE_NM) {
				$q = $p;
				while($this->isdigitat($this->dataStr, $q)) {
					$q++;
				}
			...
			} elseif ($mode == COLISSIMO_QR_MODE_AN) {
				$q = $p;
		...
		$run = $p;
		$this->items = $this->appendNewInputItem($this->items, COLISSIMO_QR_MODE_8B, $run, str_split($this->dataStr));
		return $run;
	}
	 ...
	 */
	 protected function splitString() {
		while (strlen($this->dataStr) > 0) {
			...
			switch ($mode) {
				case COLISSIMO_QR_MODE_NM: {
					$length = $this->eatNum();
					break;
				}
				case COLISSIMO_QR_MODE_AN: {
					$length = $this->eatAn();
					break;
				}
				case COLISSIMO_QR_MODE_KJ: {
					if ($this->hint == COLISSIMO_QR_MODE_KJ) {
	 ...
	 */
	 protected function encodeModeNum($inputitem, $version) {
		...
		$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], $this->lengthIndicator(COLISSIMO_QR_MODE_NM, $version), $inputitem['size']);
		for ($i=0; $i < $words; ++$i) {
	 ...
	 */
	 protected function encodeModeAn($inputitem, $version) {
		...
		$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], $this->lengthIndicator(COLISSIMO_QR_MODE_AN, $version), $inputitem['size']);
		for ($i=0; $i < $words; ++$i) {
	 ...
	 */
	 protected function encodeMode8($inputitem, $version) {
		...
		$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], $this->lengthIndicator(COLISSIMO_QR_MODE_8B, $version), $inputitem['size']);
	 ...
	 */
	 protected function encodeModeKanji($inputitem, $version) {
		...
		$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], $this->lengthIndicator(COLISSIMO_QR_MODE_KJ, $version), (int)($inputitem['size'] / 2));
	 ...
	 */
	 protected function insertStructuredAppendHeader($items, $size, $index, $parity) {
		if ($size > COLISSIMO_MAX_STRUCTURED_SYMBOLS) {
			return -1;
		}
		if (($index <= 0) OR ($index > COLISSIMO_MAX_STRUCTURED_SYMBOLS)) {
			return -1;
		}
		$buf = array($size, $index, $parity);
		$entry = $this->newInputItem(COLISSIMO_QR_MODE_ST, 3, $buf);
	 ...
	 */
	 protected function calcParity($items) {
		...
		foreach ($items as $item) {
			if ($item['mode'] != COLISSIMO_QR_MODE_ST) {
	 ...
	 protected function estimateBitStreamSize($items, $version) {
		...
		foreach ($items as $item) {
			switch($item['mode']) {
				...
				case COLISSIMO_QR_MODE_ST: {
					return COLISSIMO_STRUCTURE_HEADER_BITS;
				}
				default: {
					return 0;
				}
	...
	protected function getDataLength($version, $level) {
		return $this->capacity[$version][COLISSIMO_QRCAP_WORDS] - $this->capacity[$version][COLISSIMO_QRCAP_EC][$level];
	}
	...
	protected function getECCLength($version, $level){
		return $this->capacity[$version][COLISSIMO_QRCAP_EC][$level];
	}
	...
	protected function getWidth($version) {
		return $this->capacity[$version][COLISSIMO_QRCAP_WIDTH];
	}
	...
	protected function getRemainder($version) {
		return $this->capacity[$version][COLISSIMO_QRCAP_REMINDER];
	}
	...
	protected function getMinimumVersion($size, $level) {
		for ($i = 1; $i <= COLISSIMO_QRSPEC_VERSION_MAX; ++$i) {
			$words = ($this->capacity[$i][COLISSIMO_QRCAP_WORDS] - $this->capacity[$i][COLISSIMO_QRCAP_EC][$level]);
			if ($words >= $size) {
				return $i;
			}
		}
	...
	protected function getVersionPattern($version) {
		if (($version < 7) OR ($version > COLISSIMO_QRSPEC_VERSION_MAX)) {
			return 0;
		}
	...
	protected function createFrame($version) {
		$width = $this->capacity[$version][COLISSIMO_QRCAP_WIDTH];
	...
	protected function newFrame($version) {
		if (($version < 1) OR ($version > COLISSIMO_QRSPEC_VERSION_MAX)) {
			return NULL;
		}

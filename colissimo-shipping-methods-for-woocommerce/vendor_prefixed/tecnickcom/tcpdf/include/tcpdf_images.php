...
//
// Description :
//   Static image methods used by the ColissimoTCPDF class.
//
//============================================================+

/**
 * @file
 * This is a PHP class that contains static image methods for the ColissimoTCPDF class.<br>
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	public static function _parsejpeg($file) {
		...
		if (!@ColissimoTCPDF_STATIC::file_exists($file)) {
			return false;
		}
		$a = getimagesize($file);
		if (empty($a)) {
		...
		while (($pos = strpos($data, "ICC_PROFILE\0", $offset)) !== false) {
			...
			$length = (ColissimoTCPDF_STATIC::_getUSHORT($data, ($pos - 2)) - 16);
	...
	public static function _parsepng($file) {
		...
		$w = ColissimoTCPDF_STATIC::_freadint($f);
		$h = ColissimoTCPDF_STATIC::_freadint($f);
		$bpc = ord(fread($f, 1));
		$ct = ord(fread($f, 1));
		if ($ct == 0) {
			$colspace = 'DeviceGray';
		...
		$data = '';
		$icc = false;
		$n = ColissimoTCPDF_STATIC::_freadint($f);
		do {
			$type = fread($f, 4);
			if ($type == 'PLTE') {
				// read palette
				$pal = ColissimoTCPDF_STATIC::rfread($f, $n);
				fread($f, 4);
			} elseif ($type == 'tRNS') {
				// read transparency info
				$t = ColissimoTCPDF_STATIC::rfread($f, $n);
				if ($ct == 0) { // DeviceGray
					$trns = array(ord($t[1]));
				...
				$data .= ColissimoTCPDF_STATIC::rfread($f, $n);
				fread($f, 4);
			} elseif ($type == 'iCCP') {
				// skip profile name
				$len = 0;
				...
				$icc = ColissimoTCPDF_STATIC::rfread($f, ($n - $len - 2));
				// decompress profile
				...
				ColissimoTCPDF_STATIC::rfread($f, $n + 4);
			}
			$n = ColissimoTCPDF_STATIC::_freadint($f);
		} while ($n);
...
} // END OF ColissimoTCPDF_IMAGES CLASS
